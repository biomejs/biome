...
use super::sort_config::{UtilityLayer, Variant, build_variant_weight};
`;
...
// Generate Variants using their name and the bit representation of the BigInt weight
function generateVariants(variants: SortConfig["variants"]) {
	let output = `pub fn get_variant_classes() -> Vec<Variant> {\n`
	output += `${INDENT}vec![\n`

	for (const { name, weight } of variants) {
		const weightBits = weight.toString(2);
		const bitsLength = weightBits.length;

		output += `${INDENT}${INDENT}Variant {\n`;
		output += `${INDENT}${INDENT}${INDENT}name: "${name}",\n`;
		output += `${INDENT}${INDENT}${INDENT}weight: build_variant_weight::<${bitsLength-1}>(),\n`;
