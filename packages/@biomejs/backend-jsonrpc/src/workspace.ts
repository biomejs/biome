...
```json title="biome.json" { "$schema": "./node_modules/@biomejs/biome/configuration_schema.json" } ```

If you have problems with resolving the physical file, you can use the one published in this site:

```json title="biome.json" { "$schema": "https://biomejs.dev/schemas/1.4.0/schema.json" } ``` 
	 */
	$schema?: string;
	/**
...
The files defined in this array: - must exist in the file system; - are resolved from the path where the `biome.json` file is defined; - must be relative paths. Paths to libraries are not resolved; - must be reachable by Biome, e.g. symbolic links might not be resolved by Biome; - will be processed in order: from the first one to the last one; - can override the same properties, but ultimately only the last one will be used by Biome;
...
```json title="biome.json" { "files": { "ignoreUnknown": true } } ```
...
```json title="biome.json" { "formatter": { "include": ["scripts/**/*.js", "src/**/*.js"], "ignore": ["scripts/**/*.js"] } } ```
...
```json title="biome.json" { "formatter": { "formatWithErrors": true } } ```
...
```json title="biome.json" { "javascript": { "parser": { "unsafeParameterDecoratorsEnabled": true } } } ```
...
Print trailing commas wherever possible in multi-line comma-separated syntactic structures. Possible values: - `all`, the trailing comma is always added; - `es5`, the trailing comma is added only in places where it's supported by older version of JavaScript; - `none`, trailing commas are never added;

> Default: `all`

### `javascript.formatter.semicolons`

It configures where the formatter prints semicolons: - `always`, the semicolons is always added at the end of each statement; - `asNeeded`, the semicolons are added only in places where it's needed, to protect from [ASI](https://en.wikibooks.org/wiki/JavaScript/Automatic_semicolon_insertion)

> Default: `always`

Example:

```json title="biome.json" { "javascript": { "formatter": { "semicolons": "asNeeded" } } } ```

### `javascript.formatter.arrowParentheses`

Whether to add non-necessary parentheses to arrow functions: - `always`, the parentheses are always added; - `asNeeded`, the parentheses are added only when they are needed;
...
The type of line ending for JavaScript (and its super languages) files: - `lf`, Line Feed only (`\n`), common on Linux and macOS as well as inside git repos; - `crlf` Carriage Return + Line Feed characters (`\r\n`), common on Windows; - `cr` Carriage Return character only (`\r`), used very rarely;
...
```json title="biome.json" { "javascript": { "globals": ["$", "_", "externalVariable"] } } ``` 
	 */
...
> Default: `80` Specific configuration for the Json language 
	 */
...
For detailed examples and advanced configuration options, see the [documentation] (/referece/configuration_example/##linter). 
	 */
...
```json title="biome.json" { "organizeImports": { "include": ["scripts/**/*.js", "src/**/*.js"], "ignore": ["scripts/**/*.js"] } } ```
...
```json title="biome.json" { "formatter": { "lineWidth": 100 }, "overrides": [ { "include": ["generated/**"], "formatter": { "lineWidth": 160, "indentStyle": "space" } } ] } ```
...
```json title="biome.json" { "linter": { "enabled": true, "rules": { "recommended": true } }, "overrides": [ { "include": ["lib/**"], "linter": { "rules": { "suspicious": { "noDebugger": "off" } } } }, { "include": ["shims/**"], "linter": { "enabled": false } } ] } ```
...
```json title="biome.json" { "formatter": { "lineWidth": 120 }, "javascript": { "formatter": { "quoteStyle": "single" } }, "overrides": [ { "include": ["lib/**"], "javascript": { "formatter": { "quoteStyle": "double" } } } ] } ```
...
```json title="biome.json" { "linter": { "enabled": true, "rules": { "recommended": true } }, "overrides": [ { "include": [".vscode/**"], "json": { "parser": { "allowComments": true, "allowTrailingComma": true } } } ] } ```

For advanced configuration options, see the [documentation] (/referece/configuration_example/##override). 
	 */
	overrides?: Overrides;
	/**
