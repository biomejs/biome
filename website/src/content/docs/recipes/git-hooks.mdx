...
you can automatically set up _Lefthook_ hooks upon package installation using `scripts.postinstall`:
...
Husky doesn't hide unstaged changes and is not able to provides the list of staged files.
This is why it is often used in tandem with another tool such as _lint-staged_ or _git-format-staged_.

If your project contains a `package.json`,
you can automatically set up _husky_ hooks upon package installation using `scripts.postinstall`:
...
[lint-staged](https://github.com/lint-staged/lint-staged) is one of the most used tool in the JavaScript ecosystem.
