...
This behavior can inadvertently lead to variables with an <code>any</code> type, weakening type safety.
Just like the <code>any</code> type, evolved <code>any</code> types disable many type checking rules and should be avoided to maintain strong type safety.
This rule prevents such cases by ensuring variables do not evolve into <code>any</code> type, encouraging explicit type annotations and controlled type evolutions. |  |
