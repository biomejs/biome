...
| [noMisleadingCharacterClass](/linter/rules/no-misleading-character-class) | Disallow characters which are made with multiple code points in character class syntax | <span aria-label="The rule has a safe fix" role="img" title="The rule has a safe fix">🔧 </span> |
