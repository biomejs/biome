...
| [useLiteralEnumMembers](/linter/rules/use-literal-enum-members) | Require all enum members to be literal values. | <span aria-label="Recommended" role="img" title="Recommended">👌 </span><span aria-label="The rule has an unsafe fix" role="img" title="The rule has an unsafe fix">⚠️ </span> |
