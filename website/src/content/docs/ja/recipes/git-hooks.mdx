...
- コミットをする前にフォーマットチェックやリントを行い、[安全な修正](../linter#安全な修正safe-fixes)を行う
...
`--files-ignore-unknown=true` を指定するだけでBiomeが現在サポートしている、もしくは将来サポートするかもしれないファイルのみを対象とすることができます。
どのファイルをBiomeの対象にするかをもっとコントロールしたい場合は `glob` を使ってください。
...
Husky はステージングされていない変更を無視したり、ステージングされているファイルののみを対象とすることができません。
...
これにより、ステージングされていない変更とステージングされた変更の間で衝突が発生した場合に手動で介入する必要がなくなります。
...
Biome がすでに `npm` のパッケージとしてローカルリポジトリにインストールされている場合、
...
pre-commit の `files` オプションは必須ではありません。
