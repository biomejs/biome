...
Biomeをダウンロードする最も速い方法は、 `npm` またはお好みのパッケージマネージャーを使用することです。Node.jsをインストールせずにBiomeを使用したい場合は、[スタンドアロン実行形式](/guides/manual-installation)としてCLIを利用できます。
...
`linter.enabled: true` はLinterを有効にし、 `rules.recommended: true` は[推奨ルール](/linter/rules/)を有効にする指定です。

この設定では、`formatter.enabled: false` で明示的にフォーマットを[無効化](/reference/configuration/#formatterenabled)していないため、フォーマットは有効になっています。
