---
title: Analyzer
description: Biome analyzerが提供するもの
---

Biome analyzerはユーザーが利用できるいくつもの機能を提供します。

## インポートのソート

Biomeはインポートステートメントを[自然順](https://ja.wikipedia.org/wiki/自然順)に従ってソート出来ます。
...
インポートステートメントは "距離" によってソートされます。ユーザーから "遠い" モジュールが上に配置され、ユーザーに '近い' モジュールが下に配置されます。
...
ソートを適用する方法は[CLI](#import-sorting-via-cli)または[VSCode extension](#import-sorting-via-vscode-extension)の2つあります。

### グループインポート

特定の順序でインポートステートメントを宣言することは一般的です。主にフロントエンドのプロジェクトで作業する場合や、CSSファイルをインポートする際に顕著です。
...
他の一般的なケースは、ポリフィルやシムファイルをインポートすることです。これらはファイルの先頭に配置する必要があります。
...
こういったケースではBiomeがインポートをすべてソートすることにより、アプリケーションを**壊す**可能性があります。

これを回避するためにはインポートの"グループ"を作成します。**改行**を追加することでグループに区切ることができます。

これによりBiomeはソートをグループに属するインポートステートメントに制限します。
...
`check`コマンドを使用して、オプション`--apply`を指定します。
...
BiomeのVS Code拡張機能は、_Organize Imports_ コードアクションを介してインポートのソートをサポートしています。
