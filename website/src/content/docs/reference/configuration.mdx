...
:::caution
When both `include` and `exclude` are specified, the field `exclude` takes **precedence** over `include`
:::
...
Use this configuration to change the behaviour of the tools for certain files. The overrides are absolute, and they have precedence over the top-level configuration.

When a file is matched against an override pattern, **only** the configuration specified in that pattern will be applied. If not configuration is set, Biome's defaults are applied.
...
It will include the options of [top level formatter](#formatter), minus `ignore` and `include`.

### `overrides.<ITEM>.linter`

It will include the options of [top level formatter](#linter), minus `ignore` and `include`.

### `overrides.<ITEM>.organizeImports`

It will include the options of [top level formatter](#organize_imports), minus `ignore` and `include`.

### `overrides.<ITEM>.javascript`

It will include the options of [top level formatter](#javascript).

### `overrides.<ITEM>.json`

It will include the options of [top level formatter](#json).
