...
Se o seu projeto possuí um `package.json`,
você pode configurar automaticamente os hooks do _Lefthook_ após a instalação do pacote usando `scripts.postinstall`:
...
você pode configurar automaticamente os hooks do _husky_ após a instalação do pacote usando `scripts.postinstall`:
