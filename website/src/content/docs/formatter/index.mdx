...
As mentioned in a [previous divergence](https://github.com/biomejs/biome/issues/739#issuecomment-1815374273), Prettier's Babel-based parsing for JS/TS is very loose and allows multiple errors to be ignored. Biome's parser intentionally has stricter rules than Prettier and correctly asserts that the `const` modifier is not allowed for type parameters of interfaces.
...
We could adopt the same strategy that discussed in a [previous comment](https://github.com/biomejs/biome/issues/739#issuecomment-1815374273) to fix the divergence.
