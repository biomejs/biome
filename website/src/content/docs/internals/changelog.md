...
- Add rule [noConstantMathMinMaxClamp](https://biomejs.dev/linter/rules/no-constant-math-min-max-clamp) the use of `Math.min` and `Math.max` to clamp a value where the result itself is constant. Contributed by @mgomulak
