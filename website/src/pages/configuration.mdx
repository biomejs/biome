...
emoji: ⚙️
category: reference
description: How to customize and configure Biome with rome.json.
---
...
The Biome configuration file is named `rome.json` and should be placed in the root directory of your project. The root
...
Biome uses auto discovery to find the nearest `rome.json` file. It starts looking for `rome.json` in the current
working directory, and then it starts looking in the parent directories until:
- it finds a `rome.json` file;
- it applies Biome's defaults if **no `rome.json` is found**;
...
- are resolved from the path where the `rome.json` file is defined;
