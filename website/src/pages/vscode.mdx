...
You can install the code extension by heading to the extension's [Visual Studio Code Market Place page](https://marketplace.visualstudio.com/items?itemName=biomej.biome) or from within VS Code by either:
...
Biome respects VS Code's _Code Actions On Save_ setting. To enable fix on save, add `"editor.codeActionsOnSave": { "quickfix.rome": true }` in vscode settings.json.
...
Disables formatting, linting, and syntax errors for projects without a `rome.json` file. Requires Biome 12 or newer.
Enabled by default.
