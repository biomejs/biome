...
fn generate_code(manifests: BTreeMap<&str, ManifestModule>) -> proc_macro2::TokenStream {
    let manifests = manifests.iter().map(|(manifest_name, manifest)| {
        let manifest_name = Literal::string(manifest_name);

        let mappings = manifest.mappings.iter().map(|(mapping_key, mapping)| {
        ...
        let replacements = manifest
            .replacements
            .iter()
            .map(|(replacement_key, replacement)| {
...
pub fn generate_module_replacements(mode: Mode) -> Result<()> {
    ...
    let output_path =
        project_root().join("crates/biome_json_analyze/src/module_replacements_data.rs");
