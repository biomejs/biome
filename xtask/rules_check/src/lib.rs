...
pub fn check_rules() -> anyhow::Result<()> {
    ...
    biome_tailwind_analyze::visit_registry(&mut visitor);
...
fn assert_lint(
    ...
    match document_file_source {
        ...
        DocumentFileSource::Unknown if test.tag == "tailwind" => {
            let parse = biome_tailwind_parser::parse_tailwind(code);
            ...
            if parse.has_errors() {
                ...
                let options =
                    biome_analyze::AnalyzerOptions::default().with_file_path(test.file_path());

                biome_tailwind_analyze::analyze(&root, filter, &options, |signal| {
                    if let Some(mut diag) = signal.diagnostic() {
                        for action in signal.actions() {
                            if !action.is_suppression() {
                                diag = diag.add_code_suggestion(action.into());
