    ...
    L: ServiceLanguage,
{
    ...
    let settings = workspace_settings.get_settings_based_on_path(project_key);
...
fn assert_lint(
    ...
    if test.use_options {
        ...
        if let Some(mut settings) = workspace_settings.get_settings_based_on_path(project_key) {
