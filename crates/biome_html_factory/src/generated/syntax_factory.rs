...
impl SyntaxFactory for HtmlSyntaxFactory {
    ...
    fn make_syntax(
        ...
        match kind {
            ...
            ANGULAR_AFTER_TIME_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<2usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                    && element.kind() == T![after]
                {
                    slots.mark_present();
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_AFTER_TIME_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_AFTER_TIME_CLAUSE, children)
            }
            ...
            ANGULAR_DEFER_BLOCK => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                    && AngularDeferOpeningBlock::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularPlaceholderClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularLoadingClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularErrorClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_BLOCK, children)
            }
            ANGULAR_DEFER_HYDRATE_NEVER_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_HYDRATE_NEVER_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_HYDRATE_NEVER_CLAUSE, children)
            }
            ANGULAR_DEFER_HYDRATE_ON_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_HYDRATE_ON_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_HYDRATE_ON_CLAUSE, children)
            }
            ANGULAR_DEFER_HYDRATE_WHEN_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_HYDRATE_WHEN_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_HYDRATE_WHEN_CLAUSE, children)
            }
            ANGULAR_DEFER_ON_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_ON_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_ON_CLAUSE, children)
            }
            ANGULAR_DEFER_OPENING_BLOCK => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<4usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularDeferParameters::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_OPENING_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_OPENING_BLOCK, children)
            }
            ANGULAR_DEFER_PARAMETERS => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularDeferClauseList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_PARAMETERS.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_PARAMETERS, children)
            }
            ANGULAR_DEFER_PREFETCH_ON_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_PREFETCH_ON_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_PREFETCH_ON_CLAUSE, children)
            }
            ANGULAR_DEFER_PREFETCH_WHEN_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_PREFETCH_WHEN_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_PREFETCH_WHEN_CLAUSE, children)
            }
            ANGULAR_DEFER_WHEN_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_DEFER_WHEN_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_DEFER_WHEN_CLAUSE, children)
            }
            ANGULAR_ELSE_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_ELSE_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_ELSE_CLAUSE, children)
            }
            ANGULAR_ELSE_IF_CLAUSE => {
                let mut elements = (&children).into_iter();
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularIfParameters::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_ELSE_IF_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_ELSE_IF_CLAUSE, children)
            }
            ANGULAR_EMPTY_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_EMPTY_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_EMPTY_CLAUSE, children)
            }
            ANGULAR_ERROR_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_ERROR_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_ERROR_CLAUSE, children)
            }
            ...
            ANGULAR_FOR_BLOCK => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                    && AngularForOpeningBlock::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularEmptyClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_FOR_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_FOR_BLOCK, children)
            }
            ANGULAR_FOR_EXPRESSION => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_FOR_EXPRESSION.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_FOR_EXPRESSION, children)
            }
            ANGULAR_FOR_LET_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularForLetBindingList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_FOR_LET_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_FOR_LET_CLAUSE, children)
            }
            ANGULAR_FOR_OPENING_BLOCK => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<4usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularForParameters::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_FOR_OPENING_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_FOR_OPENING_BLOCK, children)
            }
            ANGULAR_FOR_PARAMETERS => {
                let mut elements = (&children).into_iter();
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularForExpression::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularForTrackClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularForLetClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_FOR_PARAMETERS.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_FOR_PARAMETERS, children)
            }
            ANGULAR_FOR_TRACK_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<2usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                    && element.kind() == T![track]
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_FOR_TRACK_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_FOR_TRACK_CLAUSE, children)
            }
            ANGULAR_IF_AS_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_IF_AS_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_IF_AS_CLAUSE, children)
            }
            ANGULAR_IF_BLOCK => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                    && AngularIfOpeningBlock::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularElseIfClauseList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularElseClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_IF_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_IF_BLOCK, children)
            }
            ANGULAR_IF_OPENING_BLOCK => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<4usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_IF_OPENING_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_IF_OPENING_BLOCK, children)
            }
            ANGULAR_IF_PARAMETERS => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<4usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularIfAsClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_IF_PARAMETERS.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_IF_PARAMETERS, children)
            }
            ANGULAR_LET_BLOCK => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<5usize> = RawNodeSlots::default();
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularLetInitializerClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_LET_BLOCK.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_LET_BLOCK, children)
            }
            ANGULAR_LET_INITIALIZER_CLAUSE => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_LET_INITIALIZER_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_LET_INITIALIZER_CLAUSE, children)
            }
            ANGULAR_LOADING_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<4usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularLoadingParameters::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_LOADING_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_LOADING_CLAUSE, children)
            }
            ANGULAR_LOADING_PARAMETERS => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularLoadingParameterClauseList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_LOADING_PARAMETERS.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_LOADING_PARAMETERS, children)
            }
            ANGULAR_MINIMUM_TIME_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<2usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                    && element.kind() == T![minimum]
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_MINIMUM_TIME_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_MINIMUM_TIME_CLAUSE, children)
            }
            ANGULAR_PLACEHOLDER_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<4usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularPlaceholderParameters::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_PLACEHOLDER_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_PLACEHOLDER_CLAUSE, children)
            }
            ANGULAR_PLACEHOLDER_PARAMETERS => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AngularMinimumTimeClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        ANGULAR_PLACEHOLDER_PARAMETERS.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(ANGULAR_PLACEHOLDER_PARAMETERS, children)
            }
            ...
            ANGULAR_DEFER_CLAUSE_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                AnyAngularDeferClause::can_cast,
                T ! [;],
                true,
            ),
            ...
            ANGULAR_FOR_LET_BINDING_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                HtmlTextExpression::can_cast,
                T ! [,],
                false,
            ),
            ANGULAR_LOADING_PARAMETER_CLAUSE_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                AnyAngularLoadingParameterClause::can_cast,
                T ! [;],
                true,
            ),
            HTML_ATTRIBUTE_LIST => {
