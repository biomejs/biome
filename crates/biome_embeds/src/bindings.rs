...
pub fn get_binding_by_name<'db>(
    ...
    for bindings in embedded_bindings_from_source(db, source) {
...
mod tests {
    ...
    fn parse_vue_source<'db>(db: &'db TestDb, source: &str) -> (Utf8PathBuf, EmbeddedSource<'db>) {
        let path = Utf8PathBuf::from("src/App.vue");
        let parsed: AnyParse = parse_html(source, HtmlParserOptions::default().with_vue()).into();
        let file = FileSource::new(db, path.clone(), source.to_string(), 0, None);
        db.insert_file(path.clone(), file);
        let source = EmbeddedSource::new(db, file, parsed, vec![]);
        (path, source)
    }

    fn parse_html_source_with_js_snippet<'db>(
        db: &'db TestDb,
        html: &str,
        js: &str,
    ) -> (Utf8PathBuf, EmbeddedSource<'db>) {
        let path = Utf8PathBuf::from("src/file.html");
        let parsed: AnyParse = parse_html(html, HtmlParserOptions::default()).into();
        let snippet_parse =
        ...
        let snippet = ParsedSnippet {
            ...
            element_range: TextRange::new(TextSize::from(0), TextSize::from(html.len() as u32)),
            content_range: TextRange::new(content_start, content_end),
            content_offset: content_start,
            document_source_index: Some(3),
        };
        let file = FileSource::new(db, path.clone(), html.to_string(), 2, None);
        db.insert_file(path.clone(), file);
    ...
    fn parse_vue_source_with_js_snippet<'db>(
        ...
        let path = Utf8PathBuf::from("src/App.vue");
        let parsed: AnyParse = parse_html(html, HtmlParserOptions::default().with_vue()).into();
        let snippet_parse =
        ...
        let file = FileSource::new(db, path.clone(), html.to_string(), 0, None);
        db.insert_file(path.clone(), file);
        let source = EmbeddedSource::new(db, file, parsed, vec![snippet]);
        (path, source)
    }
    ...
    fn get_binding_by_name_finds_matching_binding() {
        let db = TestDb::new();
        let (path, source) = parse_vue_source(
            &db,
    ...
    fn get_binding_with_source_finds_imported_binding_from_source_snippet() {
        let db = TestDb::new();
        let (path, source) = parse_vue_source_with_js_snippet(
            &db,
            "<script setup></script>",
    ...
    fn collects_bindings_from_plain_html_script_source_snippet() {
        ...
        let (path, source) = parse_html_source_with_js_snippet(
            &db,
    ...
    fn get_binding_by_name_is_memoized() {
        let db = TestDb::new();
        let (_, source) = parse_vue_source(
            &db,
            r#"<template><div v-for="Local in items" /></template>"#,
        );

        let _ = embedded_bindings_from_source(&db, source);

        db.clear_salsa_events();
        let _ = embedded_bindings_from_source(&db, source);
        let events = db.take_salsa_events();

        assert_function_query_was_not_run(&db, embedded_bindings_from_source, source, &events);
    }
}
