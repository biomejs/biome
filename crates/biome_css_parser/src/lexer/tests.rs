...
use biome_parser::lexer::{BufferedLexer, Lexer, LexerWithCheckpoint};
...
#[test]
fn buffered_lookahead_extends_after_trivia_and_consumption() {
    let mut buffered = BufferedLexer::new(CssLexer::from_str("a /*c*/\nb:c;"));

    assert_eq!(
        buffered.next_token(CssLexContext::Regular),
        CssSyntaxKind::IDENT
    );
    assert_eq!(
        buffered
    ...
    assert_eq!(buffered.current(), CssSyntaxKind::IDENT);
    assert_eq!(
    ...
    for (n, kind, start, end) in [
        (1, CssSyntaxKind::IDENT, 8, 9),
        (2, T![:], 9, 10),
        (3, CssSyntaxKind::IDENT, 10, 11),
        (1, CssSyntaxKind::IDENT, 8, 9),
    ] {
        let token = buffered.nth_non_trivia(n).unwrap();
        assert_eq!(token.kind(), kind);
        assert_eq!(
            token.text_range(),
            TextRange::new(TextSize::from(start), TextSize::from(end))
        );
        assert_eq!(token.has_preceding_line_break(), n == 1);
        assert_eq!(token.has_preceding_whitespace(), n == 1);
    }
    assert_eq!(buffered.current(), CssSyntaxKind::WHITESPACE);
    assert_eq!(
        buffered.current_range(),
    ...
    for kind in [
        ...
        assert_eq!(buffered.next_token(CssLexContext::Regular), kind);
    }
    assert!(buffered.has_preceding_line_break());
    assert!(buffered.has_preceding_whitespace());
    for (n, kind, start, end) in [(3, T![;], 11, 12), (4, EOF, 12, 12), (4, EOF, 12, 12)] {
        let token = buffered.nth_non_trivia(n).unwrap();
        assert_eq!(token.kind(), kind);
        assert_eq!(
            token.text_range(),
    ...
    assert!(buffered.nth_non_trivia(5).is_none());
    assert!(buffered.nth_non_trivia(5).is_none());
    assert_eq!(buffered.current(), CssSyntaxKind::IDENT);
    assert_eq!(
        buffered.current_range(),
    ...
    for kind in [T![:], CssSyntaxKind::IDENT, T![;], EOF] {
        assert_eq!(buffered.next_token(CssLexContext::Regular), kind);
    }
    assert!(buffered.nth_non_trivia(1).is_none());
    assert!(buffered.nth_non_trivia(1).is_none());
    assert!(buffered.finish().is_empty());
}

#[test]
fn buffered_lookahead_rewind_restores_tokens_and_diagnostics() {
    let mut buffered = BufferedLexer::new(CssLexer::from_str("a`b;"));
    ...
    for (n, kind, start, end) in [
        (1, CssSyntaxKind::ERROR_TOKEN, 1, 2),
        (2, CssSyntaxKind::IDENT, 2, 3),
    ] {
        let token = buffered.nth_non_trivia(n).unwrap();
        assert_eq!(token.kind(), kind);
        assert_eq!(
    ...
    assert_eq!(buffered.finish().len(), 1);
}

#[test]
fn buffered_lookahead_context_change_discards_tokens_and_diagnostics() {
    let mut buffered = BufferedLexer::new(CssLexer::from_str("a`b;"));
    ...
    assert_eq!(
        buffered.nth_non_trivia(1).unwrap().kind(),
        CssSyntaxKind::ERROR_TOKEN
    );
    assert_eq!(
        buffered.nth_non_trivia(2).unwrap().kind(),
        CssSyntaxKind::IDENT
    );
    assert_eq!(
        buffered.next_token(CssLexContext::CustomPropertyValue(
            CssCustomPropertyCommentMode::PreserveDoubleSlash,
        )),
        CssSyntaxKind::CSS_DELIM_LITERAL
    );

    for (n, kind, start, end) in [(1, CssSyntaxKind::IDENT, 2, 3), (2, T![;], 3, 4)] {
        let token = buffered.nth_non_trivia(n).unwrap();
        assert_eq!(token.kind(), kind);
        assert_eq!(
            token.text_range(),
    ...
    assert_eq!(buffered.current(), CssSyntaxKind::CSS_DELIM_LITERAL);
    assert_eq!(
        buffered.current_range(),
        TextRange::new(TextSize::from(1), TextSize::from(2))
    );
