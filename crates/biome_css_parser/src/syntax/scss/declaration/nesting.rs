...
mod tests {
    ...
    use biome_css_syntax::CssSyntaxKind::{
        CSS_DECLARATION_WITH_SEMICOLON, SCSS_NESTING_DECLARATION,
    };
    use biome_css_syntax::T;
    use biome_parser::Parser;

    fn scss_parser(source: &str) -> CssParser<'_> {
        CssParser::new(source, CssFileSource::scss(), CssParserOptions::default())
    }

    #[test]
    fn try_parse_rejects_selector_like_nested_block_without_whitespace() {
        let mut p = scss_parser("font:bold { color: red; }");
    ...
    #[test]
    fn try_parse_accepts_nested_block_with_whitespace_after_colon() {
        let mut p = scss_parser("font: bold { color: red; }");

        let syntax = try_parse_scss_nesting_declaration(&mut p, T!['}']).unwrap();

        assert_eq!(syntax.kind(&p), Some(SCSS_NESTING_DECLARATION));
        assert_eq!(p.cur_text(), "");
    }

    #[test]
    fn try_parse_accepts_regular_declaration_before_end_kind() {
        let mut p = scss_parser("font: bold }");

        let syntax = try_parse_scss_nesting_declaration(&mut p, T!['}']).unwrap();

        assert_eq!(syntax.kind(&p), Some(CSS_DECLARATION_WITH_SEMICOLON));
        assert_eq!(p.cur_text(), "}");
    }

    #[test]
    fn try_parse_rejects_regular_declaration_without_statement_boundary() {
        let mut p = scss_parser("font: bold");
        let start = p.cur_range();
    ...
    #[test]
    fn try_parse_rejects_interpolated_name_without_colon_and_rewinds() {
        let mut p = scss_parser("#{$name}");
        let start = p.cur_range();

        assert!(is_at_scss_nesting_declaration(&mut p));
        assert!(try_parse_scss_nesting_declaration(&mut p, T!['}']).is_err());
        assert_eq!(p.cur_range(), start);
        assert_eq!(p.cur_text(), "#");
    }
    ...
    #[test]
    fn parse_nesting_reports_missing_value_for_regular_declaration() {
        let mut p = scss_parser("font: ;");

        let syntax = parse_scss_nesting_declaration(&mut p);
        let kind = syntax.kind(&p);
        let (_, diagnostics, _) = p.finish();

        assert_eq!(kind, Some(CSS_DECLARATION_WITH_SEMICOLON));
