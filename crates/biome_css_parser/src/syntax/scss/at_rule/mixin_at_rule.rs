...
#[inline]
fn parse_scss_parameter_list(p: &mut CssParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
...
fn parse_scss_parameter(p: &mut CssParser) -> ParsedSyntax {
    ...
    // The default value is optional for a mixin parameter, so we only parse it when `:` is present.
...
fn expected_scss_parameter(p: &CssParser, range: biome_rowan::TextRange) -> ParseDiagnostic {
    p.err_builder("Expected a mixin parameter.", range)
