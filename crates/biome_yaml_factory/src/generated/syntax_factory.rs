    ...
    fn make_syntax(
        ...
        match kind {
            ...
            YAML_ANCHOR_PROPERTY => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![anchor_property] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_BLOCK_MAPPING => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if YamlIndent::can_cast(element.kind()) {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
                ...
                slots.next_slot();
                if let Some(element) = &current_element {
                    if YamlDedent::can_cast(element.kind()) {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_DEDENT => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        YAML_DEDENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(YAML_DEDENT, children)
            }
            YAML_DIRECTIVE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<1usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![yaml_directive] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_DOUBLE_QUOTED_SCALAR => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![yaml_double_quoted_scalar] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_FOLDED_SCALAR => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![yaml_folded_scalar] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_INDENT => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        YAML_INDENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(YAML_INDENT, children)
            }
            YAML_LITERAL_SCALAR => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<1usize> = RawNodeSlots::default();
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![yaml_literal_scalar] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_PLAIN_SCALAR => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![yaml_plain_scalar] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_SINGLE_QUOTED_SCALAR => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![yaml_single_quoted_scalar] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
            ...
            YAML_TAG_PROPERTY => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if element.kind() == T![tag_property] {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
