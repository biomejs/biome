...
pub struct ClassDedupResult {
    ...
    /// Duplicate class names, ordered by when each duplicate was first detected.
    ///
...
/// assert_eq!(result.deduplicated, "  foo  bar  ");
/// ```
pub fn find_duplicate_classes(value_str: &str) -> Option<ClassDedupResult> {
    ...
    let duplicates: Vec<String> = duplicates_in_order.into_iter().map(String::from).collect();
