...
These tests are run as part of the `biome_yaml_formatter` test suite but can be
explicitely called with `cargo test -p biome_yaml_formatter --test prettier_tests`
...
output of `biome_yaml_formatter` and Prettier's own snapshots
...
2. Remove all the directories inside
	 `crates/biome_yaml_formatter/tests/specs/prettier` to ensure all obsolete tests are
	 removed
3. Go to `crates/biome_formatter_test/src/prettier` directory
4. Install prettier ``pnpm install``
5. Go to `crates/biome_yaml_formatter/tests/specs/prettier` directory
6. Run
	 `node crates/biome_yaml_formatter/tests/specs/prettier/prepare_tests.js <prettier root directory>`
