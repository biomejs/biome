...
fn set_bun_catalogs(workspace: &LocalWorkspace, project_key: ProjectKey, enabled: bool) {
    workspace
        .update_settings(UpdateSettingsParams {
            project_key,
            workspace_directory: Some(BiomePath::new("/project")),
...
#[test]
fn bun_workspace_catalogs_refresh_after_manifest_and_settings_changes() {
    const ROOT: &str = "/project/package.json";
    const PACKAGE: &str = "/project/packages/app/package.json";
    const SOURCE: &str = r#"{
        "workspaces": {"packages": ["packages/*"], "catalogs": {"react": {"react": "19.0.0"}}},
        "dependencies": {"react": "catalog:react"}
    }"#;
    let fs = MemoryFileSystem::default();
    fs.insert(ROOT.into(), SOURCE.as_bytes());
    fs.insert(
        PACKAGE.into(),
        br#"{"devDependencies":{"react":"catalog:react"}}"#,
    );
    let fs_for_updates = MemoryFileSystem::from_files(fs.files.0.clone());
    let (workspace, project_key) = setup_workspace_and_open_project(fs, "/project");
    workspace
        ...
        .unwrap();
    let assert_react = |range: &str, expected: bool| {
        for path in ["/project", "/project/packages/app"] {
            let manifest = workspace
                .project_layout
                .get_node_manifest_for_package(Utf8Path::new(path))
                .unwrap();
            assert_eq!(
                manifest.matches_dependency("react", range),
                expected,
                "{path}: {range}"
            );
        }
    };
    assert_react(">=19.0.0", false);
    set_bun_catalogs(&workspace, project_key, true);
    assert_react(">=19.0.0", true);
    set_bun_catalogs(&workspace, project_key, false);
    assert_react(">=19.0.0", false);
    set_bun_catalogs(&workspace, project_key, true);
    assert_react(">=19.0.0", true);
    ...
    workspace
        .change_file(ChangeFileParams {
            project_key,
            path: BiomePath::new(ROOT),
            content: SOURCE.replace("19.0.0", "18.3.1"),
            version: 1,
            inline_config: None,
            editor_features: None,
        })
        .unwrap();
    assert_react(">=19.0.0", false);
    assert_react("^18.0.0", true);

    for source in [
        r#"{"dependencies":{"react":"catalog:react"}}"#,
        SOURCE,
        r#"{"workspaces":{"catalogs":false},"dependencies":{"react":"catalog:react"}}"#,
    ] {
        fs_for_updates.insert(ROOT.into(), source.as_bytes());
        workspace
            .index_file(project_key, BiomePath::new(ROOT), IndexTrigger::Update)
            .unwrap();
        assert_react(">=19.0.0", source == SOURCE);
    }
    fs_for_updates.insert(ROOT.into(), SOURCE.as_bytes());
    workspace
        .index_file(project_key, BiomePath::new(ROOT), IndexTrigger::Update)
        .unwrap();
    assert_react(">=19.0.0", true);
    fs_for_updates.remove(Utf8Path::new(ROOT));
    workspace
        .unload_file(Utf8Path::new(ROOT), project_key)
        .unwrap();
    let manifest = workspace
        .project_layout
        .get_node_manifest_for_package(Utf8Path::new("/project/packages/app"))
        .unwrap();
    assert!(manifest.catalog.is_none());
}

#[test]
fn bun_workspace_catalogs_resolve_before_root_manifest_is_indexed() {
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/project/package.json".into(),
        br#"{"workspaces":["packages/*"],"catalog":{"react":"19.0.0"}}"#,
    );
    fs.insert(
        "/project/packages/app/package.json".into(),
        br#"{"peerDependencies":{"react":"catalog:"}}"#,
    );
    let (workspace, project_key) = setup_workspace_and_open_project(fs, "/project");
    set_bun_catalogs(&workspace, project_key, true);
    workspace
        .index_file(
            project_key,
            BiomePath::new("/project/packages/app/package.json"),
    ...
    let manifest = workspace
        ...
        .get_node_manifest_for_package(Utf8Path::new("/project/packages/app"))
        .unwrap();
    assert!(manifest.matches_dependency("react", ">=19.0.0"));
}

#[test]
fn workspace_catalog_options_are_independent() {
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/project/package.json".into(),
        br#"{
    ...
    fs.insert(
        "/project/pnpm-workspace.yaml".into(),
        b"catalog:\n  react: 18.3.1\n",
    );
    let fs_for_updates = MemoryFileSystem::from_files(fs.files.0.clone());
    let (workspace, project_key) = setup_workspace_and_open_project(fs, "/project");
    workspace
        .index_file(
            project_key,
            BiomePath::new("/project/package.json"),
            IndexTrigger::InitialScan,
        )
        .unwrap();
    for (pnpm, bun, expected) in [
        (false, false, None),
        (false, true, Some("19.0.0")),
        (true, false, Some("18.3.1")),
        (true, true, Some("18.3.1")),
    ] {
        workspace
            .update_settings(UpdateSettingsParams {
                project_key,
                ...
                configuration: Configuration {
                    javascript: Some(JsConfiguration {
                        resolver: Some(JsResolverConfiguration {
                            experimental_pnpm_catalogs: Some(pnpm.into()),
                            experimental_bun_catalogs: Some(bun.into()),
                        }),
                        ..Default::default()
                    }),
                    ..Default::default()
        ...
        assert_eq!(
            manifest
                .catalog
                .as_ref()
                .and_then(|catalogs| catalogs.default.as_ref())
                .and_then(|deps| deps.get("react")),
            expected
        );
    }
    fs_for_updates.remove(Utf8Path::new("/project/pnpm-workspace.yaml"));
    workspace
        .unload_file(Utf8Path::new("/project/pnpm-workspace.yaml"), project_key)
        .unwrap();
    let manifest = workspace
        .project_layout
        .get_node_manifest_for_package(Utf8Path::new("/project"))
