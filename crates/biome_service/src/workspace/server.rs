...
impl WorkspaceServer {
    ...
    ///
    /// If no top-level `biome.json` can be found, we still return the given
    /// path to allow opening a project with default settings.
    ///
    /// An error may be returned if there is a `biome.json` file that cannot be
    /// read of parsed.
...
impl Workspace for WorkspaceServer {
    ...
    fn open_project(&self, params: OpenProjectParams) -> Result<ProjectKey, WorkspaceError> {
        let path = self.find_project_root(params.path)?;
