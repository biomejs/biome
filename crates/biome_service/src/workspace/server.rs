...
use crate::file_handlers::{
    ...
    FixedFileResult, LintParams, LintResults, ParseEmbeddedCaches,
    ParseEmbeddedParams, ParseResult, ParsedSource, ResolveBindingParams, ResolveDefinitionParams,
    UpdateSnippetsNodes,
};
...
use crate::workspace::{
    ...
    CloseProjectParams, DefinitionReference, DropPatternParams, FeaturesBuilder, FileContent,
    ...
    GoToDefinitionParams, GoToDefinitionResult, IgnoreKind, LocalEmbeddedLanguage, OpenFileParams,
    OpenFileResult, OpenProjectParams, OpenProjectResult, ParsePatternParams, ParsePatternResult,
    PathIsIgnoredParams, ProcessFileParams, ProcessFileResult, PullActionsParams,
    ...
    RenameResult, ScanKind, ScanProjectParams, ScanProjectResult, SearchPatternParams, SearchQuery,
    SearchResults, ServerInfo, ServiceNotification, Settings, SupportsFeatureParams,
    UpdateModuleGraphParams, UpdateSettingsParams, UpdateSettingsResult, db,
...
use biome_js_semantic::{
    SemanticModel, SemanticModelOptions, js_semantic_model,
    semantic_model as build_js_semantic_model,
};
#[cfg(all(feature = "module_graph", feature = "lang_js"))]
...
pub struct WorkspaceServer {
    ...
    /// the mutex.
    embedded_parse_caches: Mutex<FxHashMap<Utf8PathBuf, ParseEmbeddedCaches>>,
...
impl ProcessFileState {
    ...
    fn embedded_source(
        ...
        embedded_source_data(
            &self.db,
            path,
            self.parsed.parse(),
            self.iter_snippets().cloned().collect(),
        )
    }
...
impl WorkspaceServerWithDb<'_> {
    fn finish_change_file(
        ...
        let parsed = self.get_parse(project_key, path)?;
        ...
        if self.is_indexed(path) {
            ...
            let (dependencies, diagnostics) = self.update_service_data(
                ...
                UpdateKind::AddedOrChanged {
                    reason: OpenFileReason::ClientRequest,
                    parsed,
                    file_source,
                },
                project_key,
    ...
    fn open_file_internal(
        ...
        if let Some((parsed, file_source)) = parsed {
            let (dependencies, diagnostics) = self.update_service_data(
                &path,
                UpdateKind::AddedOrChanged {
                    reason,
    ...
    fn resolve_binding_in_document_or_snippets(
        ...
        let embedded_source =
            workspace_db
                ...
                .map(
                    |file_source| biome_workspace_db::embedded::EmbeddedSourceData {
                        file_source,
                        parsed: parse.parse(),
                        snippets: snippets
                            .iter()
                            .filter_map(ParsedSource::as_snippet)
                            .cloned()
                            .collect(),
                    },
                );
        ...
        if let Some(resolve_binding) = capabilities.editors.resolve_binding {
            let result = resolve_binding(ResolveBindingParams {
                parsed_source: parse.clone().into_source(),
                cursor_offset,
        ...
        for parsed_snippet in snippets {
            ...
            let Some(file_source) = workspace_db.source_from_index(snippet.document_source_index())
            else {
                continue;
            };
    ...
    fn parse_workspace_embedded_language_snippets(
        ...
        self.with_workspace_embedded_parse_caches(path.as_path(), |caches| {
            self.parse_embedded_language_snippets(path, any_parse, file_source, settings, caches)
        })
    }
    ...
    fn with_workspace_embedded_parse_caches<T>(
        ...
        let mut caches = self
            ...
            .remove(path)
            .unwrap_or_default();
        let result = operation(&mut caches);
    ...
    fn get_parse(
        ...
        let result = self.check_file_size(CheckFileSizeParams {
            project_key,
            path: path.clone(),
        })?;
        if result.is_too_large() {
            return Err(WorkspaceError::file_ignored(path.to_string()));
        }

        let settings = self.settings_handle(&settings, None);

        self.parse(path, &settings)
    }
    ...
    fn get_parse_snippets(
        ...
        let (file, file_source) = {
            let db = self.get_db();
            db.file_and_source_from_path(path.as_path())
                .ok_or_else(|| WorkspaceError::not_found(path.to_string()))?
        };
        if !file_source.can_contain_embeds() {
            return Ok(Vec::new());
        }
        let parsed_source = self.get_parse(project_key, path)?;
        let snippets = self.parse_workspace_embedded_language_snippets(
            path,
            parsed_source.any_parse(),
            &file_source,
            &settings,
        );
    ...
    fn parse(
        ...
        let parsed = parse(&BiomePath::new(path), settings, db.into_untracked_db())?;
        Ok(ParsedSource::workspace(parsed.any_parse.into(), file))
    }
    ...
    fn process_file_state_from_server(
        ...
        let parsed = self.get_parse(project_key, path)?;
        let snippets = self.get_parse_snippets(project_key, path)?;
        let db = self.get_db().into_untracked_db();
        let file = parsed
            .workspace_file()
            .ok_or_else(|| WorkspaceError::not_found(path.to_string()))?;
    ...
    fn fix_file_state(
        ...
        #[cfg(feature = "html_embeds")]
        let original_embedded_source = state.embedded_source(&path);
        ...
        if let Some(update_snippets) = capabilities.analyzer.update_snippets {
            ...
            for parsed_snippet in &state.snippets {
                ...
                let Some(document_file_source) = state
                    .db
                    .source_from_index(embedded_snippet.document_source_index())
                else {
                    continue;
                };
                ...
                let Some(results) = fix_all(FixAllParams {
                    ...
                    #[cfg(feature = "html_embeds")]
                    embedded_source: original_embedded_source.clone(),
                    #[cfg(feature = "module_graph")]
                    ...
                    if should_format {
                        let fixed_snippet = ParsedSource::stateless(
                            ParsedSnippet {
                                ...
                                document_source_index: Some(
                                    embedded_snippet.document_source_index(),
                                ),
                            }
                            .into(),
                        );
                        let format = capabilities
                            ...
                            .format
                            .ok_or_else(self.build_capability_error(&path))?;
                        format(&path, &document_file_source, fixed_snippet, &settings)?.into_code()
                    } else {
        ...
        #[cfg(feature = "html_embeds")]
        let embedded_source = state.embedded_source(&path);
    ...
    fn format_file_state(
        ...
        let printed = if embedded_nodes.is_empty() {
            format(path, &state.file_source, state.parsed.clone(), &settings)?
        } else {
    ...
    fn pull_diagnostics_for_state(
            ...
            && let Some(lint) = capabilities.analyzer.lint
        {
            ...
            for parsed_snippet in &state.snippets {
                ...
                let Some(file_source) = state
                    .db
                    .source_from_index(embedded_node.document_source_index())
                else {
                    continue;
                };
                let capabilities = self.features.get_deprecated_capabilities(file_source);
    ...
    fn update_module_graph_internal(
        ...
        let snippets = self.extract_module_snippets(project_key, path, &update_kind)?;
        // Keep reads, resolution, and writes separated so commits never run
        // while this thread still holds a database fork.
        let inputs = {
            let db = self.get_db();
            self.extract_module_inputs(&db, &update_kind, &snippets)?
        };
    ...
    fn extract_module_snippets(
        ...
        #[cfg(all(feature = "lang_html", feature = "html_embeds"))]
        {
            ...
            match parsed.origin() {
                ParseOrigin::Workspace(_) => self.parse_workspace_embedded_language_snippets(
                    path,
                    &parse,
                    file_source,
                    &settings,
        ...
        {
            let _ = (project_key, path, update_kind);
            Ok(Vec::new())
        }
    }
                ...
                if let Some(js_root) = parsed.parse().into_language_root::<AnyJsRoot>() {
                    let semantic_model = match parsed.origin() {
                        ParseOrigin::Workspace(file) => {
                            Arc::new(js_semantic_model(db, file, parsed).clone())
                        }
                        ParseOrigin::Stateless => {
                            let js_file_source =
                                file_source.to_js_file_source().unwrap_or_default();
                            Arc::new(build_js_semantic_model(
                                &js_root,
                                SemanticModelOptions::from(&js_file_source),
                            ))
                        }
                    };
    ...
    fn update_service_data(
        ...
        let settings = self
            .project_get_settings_for_path(project_key, &path)
            .ok_or_else(WorkspaceError::no_project)?;
...
impl Workspace for WorkspaceServerWithDb<'_> {
        ...
        let paths_to_remove = {
            ...
            for path in db.file_paths() {
                if self
                    ...
                    .path_belongs_only_to_project_with_path(path.as_path(), &project_path)
                {
                    paths.push(path)
                }
            }
    ...
    fn change_file(
        ...
        if is_indexed {
            self.finish_change_file(project_key, &path)
        } else {
            Ok(ChangeFileResult {
                diagnostics: Vec::new(),
            })
    ...
    fn process_file(&self, params: ProcessFileParams) -> Result<ProcessFileResult, WorkspaceError> {
        ...
        let mut caches = ParseCaches::default();
        match content {
            FileContent::FromServer => {
                ...
                process(
                    source.as_str(),
                    self.process_file_state_from_server(project_key, &path)?,
                    true,
                    &mut caches,
                )
            }
    ...
    fn pull_diagnostics(
        ...
        let state = self.process_file_state_from_server(params.project_key, &params.path)?;
        #[cfg(feature = "module_graph")]
        let module_db = state.db.rc_module_db();
        self.pull_diagnostics_for_state(
            params,
    ...
    fn pull_diagnostics_and_actions(
        ...
        let parsed_snippets = self.get_parse_snippets(project_key, &path)?;
        ...
        let workspace_db = self.get_db();
        #[cfg(feature = "html_embeds")]
        let embedded_source = embedded_source_data(
            &workspace_db,
            &path,
            parse.parse(),
            parsed_snippets
                ...
                capabilities.analyzer.pull_diagnostics_and_actions
        {
            ...
            let mut final_result = PullDiagnosticsAndActionsResult {
                diagnostics: Vec::new(),
            };
            final_result = pull_diagnostics_and_actions(DiagnosticsAndActionsParams {
    ...
    fn format_file(&self, params: FormatFileParams) -> Result<Printed, WorkspaceError> {
        ...
        let parse = self.get_parse(params.project_key, &params.path)?;
        let parsed_snippets = self.get_parse_snippets(params.project_key, &params.path)?;
    ...
    fn fix_file(&self, params: FixFileParams) -> Result<FixFileResult, WorkspaceError> {
        ...
        let mut state = self.process_file_state_from_server(project_key, &path)?;
        let (_, settings, query) = self
    ...
    fn go_to_definition(
        ...
        let parse = self.get_parse(params.project_key, path)?;
        let parsed_snippets = self.get_parse_snippets(params.project_key, path)?;
        ...
        let (definition_ref, capabilities) = self.resolve_binding_in_document_or_snippets(
            path,
            cursor_offset,
            &parse,
            &parsed_snippets,
        ...
        Ok(resolve_definition(ResolveDefinitionParams {
            ...
            parsed_source: parse.into_source(),
        }))
    }
    ...
    fn update_module_graph(&self, params: UpdateModuleGraphParams) -> Result<(), WorkspaceError> {
        #[cfg(feature = "module_graph")]
        {
            let parsed = self.get_parse(params.project_key, &params.path)?;
            let settings = self
                .project_get_settings_for_path(params.project_key, &params.path)
                .ok_or_else(WorkspaceError::no_project)?;
            let file_source = self.get_file_source(
                &params.path,
                settings.experimental_full_html_support_enabled(),
            );
