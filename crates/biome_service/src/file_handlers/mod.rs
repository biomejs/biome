...
impl<'b> AnalyzerVisitorBuilder<'b> {
    ...
    pub(crate) fn finish(self) -> (Vec<RuleFilter<'b>>, Vec<RuleFilter<'b>>, AnalyzerOptions) {
        ...
        if let Some(path) = self.path {
            if analyzer_options.jsx_runtime() == Some(biome_analyze::options::JsxRuntime::ReactClassic) {
                if analyzer_options.jsx_factory().is_none() {
                    let factory = self.project_layout.query_tsconfig_for_path(path, |tsconfig| {
                ...
                if analyzer_options.jsx_fragment_factory().is_none() {
                    let fragment_factory = self.project_layout.query_tsconfig_for_path(path, |tsconfig| {
