...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_extraneous_dependencies: Option<RuleConfiguration<NoExtraneousDependencies>>,
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> IndexSet<RuleFilter> {
        ...
        if let Some(rule) = self.no_extraneous_dependencies.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[10]));
            }
        }
