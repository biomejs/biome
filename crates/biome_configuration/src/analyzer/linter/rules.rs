...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub all: Option<bool>,
    #[doc = "Require specifying name for GraphQL operations."]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_anonymous_operations:
        Option<RuleFixConfiguration<biome_graphql_analyze::options::NoAnonymousOperations>>,
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        let mut index_set = FxHashSet::default();
        if let Some(rule) = self.no_anonymous_operations.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[0]));
            }
        }
