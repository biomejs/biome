...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_playwright_missing_await.as_ref()
            && rule.is_enabled()
        {
            index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[19]));
        }
        ...
        if let Some(rule) = self.use_playwright_valid_describe_callback.as_ref()
            && rule.is_enabled()
        {
            index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[50]));
        }
    ...
    fn get_rule_configuration(
        ...
        match rule_name {
            ...
            "useMaxParams" => self
                ...
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "usePlaywrightValidDescribeCallback" => self
...
impl From<GroupPlainConfiguration> for Nursery {
    fn from(value: GroupPlainConfiguration) -> Self {
        Self {
            ...
            no_playwright_missing_await: Some(value.into()),
            ...
            use_playwright_valid_describe_callback: Some(value.into()),
