...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_redundant_default_export.as_ref()
            && rule.is_enabled()
        {
            index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[22]));
        }
    ...
    fn get_rule_configuration(
        ...
        match rule_name {
            ...
            "noReactForwardRef" => self
                ...
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noRedundantDefaultExport" => self
                .no_redundant_default_export
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noReturnAssign" => self
                .no_return_assign
...
impl From<GroupPlainConfiguration> for Nursery {
    fn from(value: GroupPlainConfiguration) -> Self {
        Self {
            ...
            no_redundant_default_export: Some(value.into()),
