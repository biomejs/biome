...
impl RuleName {
    ...
    pub const fn group(self) -> RuleGroup {
        match self {
            ...
            Self::NoEmptyFile => RuleGroup::Nursery,
...
impl std::str::FromStr for RuleName {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "noEmptyFile" => Ok(Self::NoEmptyFile),
...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_empty_file.as_ref()
            && rule.is_enabled()
        {
            index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[2]));
        }
