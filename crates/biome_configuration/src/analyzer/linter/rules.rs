...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_visible_task: Option<RuleConfiguration<biome_js_analyze::options::NoVisibleTask>>,
...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_visible_task.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[45]));
            }
        }
...
impl From<GroupPlainConfiguration> for Nursery {
    fn from(value: GroupPlainConfiguration) -> Self {
        Self {
            ...
            no_visible_task: Some(value.into()),
