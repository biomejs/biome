...
pub struct Nursery {
    ...
    #[doc = "Disallow using !important in declarations"]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_important: Option<RuleConfiguration<biome_css_analyze::options::NoImportant>>,
...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_important.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[19]));
            }
        }
