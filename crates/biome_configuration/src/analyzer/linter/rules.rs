...
impl RuleGroupExt for Nursery {
    ...
    fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.use_consistent_method_signatures.as_ref()
            && rule.is_enabled()
        {
            index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[44]));
        }
    ...
    fn get_rule_configuration(
        ...
        match rule_name {
            ...
            "useConsistentGraphqlDescriptions" => self
                ...
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "useConsistentMethodSignatures" => self
...
impl From<GroupPlainConfiguration> for Nursery {
    fn from(value: GroupPlainConfiguration) -> Self {
        Self {
            ...
            use_consistent_method_signatures: Some(value.into()),
