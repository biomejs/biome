...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub no_duplicated_graphql_fields: Option<RuleConfiguration<NoDuplicatedGraphqlFields>>,
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> FxHashSet<RuleFilter<'static>> {
        ...
        if let Some(rule) = self.no_duplicated_graphql_fields.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[7]));
            }
        }
    ...
    pub(crate) fn get_rule_configuration(
        ...
        match rule_name {
            ...
            "noDuplicateSelectorsKeyframeBlock" => self
                ...
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noDuplicatedGraphqlFields" => self
                .no_duplicated_graphql_fields
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noEmptyBlock" => self
                .no_empty_block
