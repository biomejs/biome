...
pub struct Nursery {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub components_return_once: Option<RuleConfiguration<ComponentsReturnOnce>>,
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> IndexSet<RuleFilter> {
        let mut index_set = IndexSet::new();
        if let Some(rule) = self.components_return_once.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[0]));
            }
        }
    ...
    pub(crate) fn get_rule_configuration(
        ...
        match rule_name {
            "componentsReturnOnce" => self
                .components_return_once
                .as_ref()
                .map(|conf| (conf.level(), conf.get_options())),
            "noBarrelFile" => self
                .no_barrel_file
