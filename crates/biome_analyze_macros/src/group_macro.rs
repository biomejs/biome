...
fn generate_group_code(category: &str, group: &str) -> Result<TokenStream> {
    ...
    let base_path = PathBuf::from(manifest_dir).join("src").join(category);

    // Discover all rules in the group directory
    let group_dir = base_path.join(group);
    let mut rules = BTreeMap::new();

    for entry in fs2::read_dir(&group_dir)
    ...
    if rules.is_empty() {
        bail!("No rules found in directory: {}", group_dir.display());
    }
