...
use biome_html_syntax::{
    AnyHtmlAttributeInitializer, HtmlAttribute, HtmlAttributeInitializerClause,
...
declare_node_union! {
    pub AnyAstroClassAttribute = HtmlAttribute | HtmlAttributeSingleTextExpression
}

impl Rule for UseAstroClassListDirective {
    type Query = Ast<AnyAstroClassAttribute>;
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        match ctx.query() {
            AnyAstroClassAttribute::HtmlAttribute(attribute) => {
            ...
            AnyAstroClassAttribute::HtmlAttributeSingleTextExpression(attribute) => {
                if attribute.syntax().parent().is_some_and(|parent| {
                ...
                let token = attribute
                    ...
                    .html_literal_token()
                    .ok()?;
                (token.text_trimmed() == "class").then(|| token.text_trimmed_range())
            }
        }
    }
