...
impl Rule for NoAstroUnsafeInlineScripts {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        if options.allow_define_vars.unwrap_or(false) && has_define_vars(&attributes) {
            return None;
        }
            ...
            .and_then(|attribute| attribute.as_static_value())
        {
            ...
            let is_allowed = options.allow_non_executing_types.as_deref().map_or_else(
                ...
                |allowed_types| {
                    allowed_types
                        .iter()
                        .any(|allowed| allowed.as_ref() == normalized_type)
                },
            );
            if is_allowed {
                return None;
...
fn has_define_vars(attributes: &biome_html_syntax::HtmlAttributeList) -> bool {
