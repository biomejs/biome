<template>
	...
	<!-- ── Class defined inside @media query ────────────────────────────── -->
	<div class="mobile">Responsive class</div>

	<!-- ── Both parts of a descendant selector ───────────────────────────── -->
	<div class="parent">
	...
	<!-- ── Both classes from a combined selector ─────────────────────────── -->
...
<style>
	...
	/* Class inside @media query */
	@media (max-width: 768px) {
		.mobile { display: block; }
	}

	/* Classes from nested selector (.parent .child — both collected) */
	.parent .child { color: red; }

	/* Class with pseudo-selector (.button is still collected) */
	.button:hover { opacity: 0.8; }

	/* Combined selector — both .active and .highlighted are collected */
	.active.highlighted { background: yellow; }
</style>

<!-- Vue allows a second <style> block (e.g. scoped) -->
<style scoped>
