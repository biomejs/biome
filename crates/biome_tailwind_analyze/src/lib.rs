...
pub(crate) type TailwindRuleAction = RuleAction<TailwindLanguage>;
...
pub fn analyze<'a, F, B>(
    root: &LanguageRoot<TailwindLanguage>,
    filter: AnalysisFilter,
    options: &'a AnalyzerOptions,
    emit_signal: F,
) -> (Option<B>, Vec<Error>)
where
    F: FnMut(&dyn AnalyzerSignal<TailwindLanguage>) -> ControlFlow<B> + 'a,
    B: 'a,
{
...
where
    V: FnMut(&MatchQueryParams<TailwindLanguage>) + 'a,
    F: FnMut(&dyn AnalyzerSignal<TailwindLanguage>) -> ControlFlow<B> + 'a,
    B: 'a,
{
    ...
    let mut analyzer = biome_analyze::Analyzer::new(
        ...
        Box::new(TailwindSuppressionAction),
        &mut emit_signal,
    );
