...
"#;
    ...
    let (fs, result) = run_cli(
        fs,
        &mut console,
        &["check", "--use-editorconfig=true", test_file.as_str()],
    );
...
"#,
    ...
    let (fs, result) = run_cli(
        fs,
        &mut console,
        &["format", "--write", "--config-path=foo", test_file.as_str()],
    );
...
"#;
    ...
    let (fs, result) = run_cli(
        fs,
        &mut console,
        &["ci", "--use-editorconfig=true", test_file.as_str()],
    );
