...
/// Formatting parses every `v-slot` value as a binding pattern, so this pins the
/// whole pipeline down. Biome does not reformat directive values today — only
/// `{{ }}` interpolations — so the values themselves must come back untouched.
#[test]
...
/// A `v-slot` value that is not a binding pattern must recover into a
/// `JsBogusBinding` inside the slot-scope root. Leaving the leftover tokens as
/// direct children of the root makes the root itself bogus, and every consumer
/// reaching for the typed root then crashes.
#[test]
