...
#[test]
fn reports_diagnostics_concise_check_command() {
...
#[test]
fn reports_diagnostics_concise_ci_command() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();
...
#[test]
fn reports_diagnostics_concise_lint_command() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "reports_diagnostics_concise_lint_command",
        fs,
        console,
        result,
    ));
...
#[test]
fn reports_diagnostics_concise_format_command() {
    let fs = MemoryFileSystem::default();
    let mut console = BufferConsole::default();
    ...
    assert_cli_snapshot(SnapshotPayload::new(
        module_path!(),
        "reports_diagnostics_concise_format_command",
        fs,
        console,
        result,
    ));
