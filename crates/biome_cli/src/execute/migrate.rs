    ...
    match sub_command {
        ...
        Some(MigrateSubCommand::Eslint {
            ...
            if let Some(biome_config) = biome_config {
                let (mut biome_eslint_config, results) =
                    eslint_config.into_biome_config(&eslint_to_biome::MigrationOptions {
                        include_inspired,
                        include_nursery,
                    });
                biome_eslint_config.merge_with(biome_config);
                let biome_config = biome_eslint_config;
                let new_content = serde_json::to_string(&biome_config).map_err(|err| {
                    CliDiagnostic::MigrateError(MigrationDiagnostic {
                        reason: err.to_string(),
                    })
                ...
                let printed = workspace.format_file(FormatFileParams { path: biome_path })?;
                if results.migrated_rules.is_empty() || biome_config_content == printed.as_code() {
                    console.log(markup! {
                ...
                if !results.inspired_rules.is_empty() {
                    console.log(markup! {
