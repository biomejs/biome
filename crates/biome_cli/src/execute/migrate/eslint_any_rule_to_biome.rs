...
pub(crate) fn migrate_eslint_any_rule(
    ...
    match eslint_name {
        ...
        "@graphql-eslint/lone-executable-definition" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .use_lone_executable_definition
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "@graphql-eslint/unique-argument-names" => {
            let group = rules.correctness.get_or_insert_with(Default::default);
            let rule = group
                .unwrap_group_as_mut()
                .no_duplicate_argument_names
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        "@graphql-eslint/unique-enum-value-names" => {
            let group = rules.correctness.get_or_insert_with(Default::default);
            let rule = group
                .unwrap_group_as_mut()
                .no_duplicate_enum_value_names
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "@graphql-eslint/unique-input-field-names" => {
            let group = rules.correctness.get_or_insert_with(Default::default);
            let rule = group
                .unwrap_group_as_mut()
                .no_duplicate_input_field_names
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        "@graphql-eslint/unique-operation-name" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .no_duplicate_graphql_operation_name
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        "@graphql-eslint/unique-variable-names" => {
            let group = rules.correctness.get_or_insert_with(Default::default);
            let rule = group
                .unwrap_group_as_mut()
                .no_duplicate_variable_names
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "@typescript-eslint/method-signature-style" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .use_consistent_method_signatures
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "@typescript-eslint/no-mixed-enums" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .use_consistent_enum_value_type
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "astro/no-exports-from-components" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .no_astro_exports_from_components
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        "astro/no-prerender-export-outside-pages" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .no_astro_prerender_export_outside_pages
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "astro/no-unsafe-inline-scripts" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .no_astro_unsafe_inline_scripts
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "astro/no-unused-define-vars-in-style" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .no_astro_unused_define_vars_in_style
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        "astro/prefer-class-list-directive" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .use_astro_class_list_directive
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
        ...
        "vue/no-setup-props-reactivity-loss" => {
            ...
            let rule = group
                .unwrap_group_as_mut()
                .no_vue_setup_props_reactivity_loss
                .get_or_insert(Default::default());
            rule.set_level(rule.level().max(rule_severity.into()));
        }
