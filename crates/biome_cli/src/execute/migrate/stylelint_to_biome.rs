...
pub(crate) struct StylelintMigrationResults {
...
impl StylelintMigrationResults {
...
impl biome_diagnostics::Diagnostic for StylelintMigrationResults {
...
) -> (biome_config::Configuration, StylelintMigrationResults) {
...
impl stylelint_stylelint::StylelintConfigData {
    pub(crate) fn into_biome_config(
        ...
        let mut results = StylelintMigrationResults::default();
        ...
        let mut rules = self
            .rules
            .into_biome_rules(default_severity, false, options, &mut results);
...
impl stylelint_stylelint::Rules {
    pub(crate) fn into_biome_rules(
        ...
        emit_disabled_as_off: bool,
        options: &MigrationOptions,
        results: &mut StylelintMigrationResults,
    ) -> biome_config::Rules {
        let mut rules = biome_config::Rules::default();
        for (name, data) in self.0 {
            let level = if data.enabled {
                data.severity.unwrap_or(default_severity).into()
            } else if emit_disabled_as_off {
                biome_config::RulePlainConfiguration::Off
            } else {
                continue;
            };
            migrate_stylelint_any_rule(&mut rules, &name, level, options, results);
        }
        rules
    }
}
