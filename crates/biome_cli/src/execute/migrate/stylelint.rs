...
fn load_config_data(
    ...
    if let Some(mut result) = deserialized {
        // Recursively resolve the `extends` field, tracking the active chain of
        // configs to detect circular references.
        let mut chain = Vec::new();
        resolve_extends(&mut result, &mut chain, console)?;
        Ok(result)
    } else {
...
/// Load and merge included configuration via `config.extends`.
///
/// Resolution is depth-first. `chain` holds the identities of the configs
/// currently being resolved along the active branch; encountering a preset that
/// is already in `chain` indicates a circular `extends` reference and is
/// reported as an error. Identities are removed once their branch completes, so
/// a preset reused by separate branches is still resolved.
///
/// Presets are combined in declaration order, each merged on top of the
/// previous, and the local configuration is merged last so that it takes
/// precedence over everything it extends.
///
/// Unknown configurations are ignored. `config.extends` is left empty.
fn resolve_extends(
    config: &mut stylelint_stylelint::StylelintConfigData,
    chain: &mut Vec<String>,
    console: &mut dyn Console,
) -> Result<(), CliDiagnostic> {
    let presets = std::mem::take(&mut config.extends);
    if presets.is_empty() {
        return Ok(());
    }
    let mut inherited = stylelint_stylelint::StylelintConfigData::default();
    for preset in presets {
        if chain.iter().any(|resolved| resolved == &preset) {
            return Err(CliDiagnostic::MigrateError(MigrationDiagnostic {
                reason: format!(
                    "Circular `extends` reference detected while resolving the Stylelint configuration: '{preset}'."
                ),
            }));
        }
        let mut extended = match load_stylelint_extends_config(&preset) {
        ...
        chain.push(preset);
        resolve_extends(&mut extended, chain, console)?;
        chain.pop();
        inherited.merge_with(extended);
    }
    // Merge the local configuration on top of the inherited configurations.
    let local = std::mem::replace(config, inherited);
    config.merge_with(local);
    Ok(())
}
