...
pub enum AnyHtmlAttribute {
    AnyAngularAttribute(AnyAngularAttribute),
...
impl AnyHtmlAttribute {
    pub fn as_any_angular_attribute(&self) -> Option<&AnyAngularAttribute> {
        match &self {
            Self::AnyAngularAttribute(item) => Some(item),
            _ => None,
        }
    }
...
impl std::fmt::Debug for AnyAngularAttribute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl From<AnyAngularAttribute> for SyntaxNode {
    fn from(n: AnyAngularAttribute) -> Self {
        match n {
            AnyAngularAttribute::AngularEventBinding(it) => it.into_syntax(),
            AnyAngularAttribute::AngularPropertyBinding(it) => it.into_syntax(),
            AnyAngularAttribute::AngularStructuralDirective(it) => it.into_syntax(),
            AnyAngularAttribute::AngularTemplateRefVariable(it) => it.into_syntax(),
            AnyAngularAttribute::AngularTwoWayBinding(it) => it.into_syntax(),
        }
    }
}
impl From<AnyAngularAttribute> for SyntaxElement {
    fn from(n: AnyAngularAttribute) -> Self {
...
impl AstNode for AnyHtmlAttribute {
    type Language = Language;
    const KIND_SET: SyntaxKindSet<Language> = AnyAngularAttribute::KIND_SET
    ...
    fn can_cast(kind: SyntaxKind) -> bool {
        match kind {
            ...
            k if AnyAngularAttribute::can_cast(k) => true,
    ...
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        let res = match syntax.kind() {
            ...
            _ => {
                let syntax = match AnyAngularAttribute::try_cast(syntax) {
                    Ok(any_angular_attribute) => {
                        return Some(Self::AnyAngularAttribute(any_angular_attribute));
                    }
                    Err(syntax) => syntax,
                };
    ...
    fn syntax(&self) -> &SyntaxNode {
        match self {
            ...
            Self::AnyAngularAttribute(it) => it.syntax(),
    ...
    fn into_syntax(self) -> SyntaxNode {
        match self {
            ...
            Self::AnyAngularAttribute(it) => it.into_syntax(),
...
impl std::fmt::Debug for AnyHtmlAttribute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::AnyAngularAttribute(it) => std::fmt::Debug::fmt(it, f),
...
impl From<AnyHtmlAttribute> for SyntaxNode {
    fn from(n: AnyHtmlAttribute) -> Self {
        match n {
            AnyHtmlAttribute::AnyAngularAttribute(it) => it.into_syntax(),
...
impl std::fmt::Display for AnyAngularAttribute {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
