...
pub struct SemanticModelBuilder {
    ...
    bindings_by_range: Vec<BindingIdentifier>,
    /// maps a reference range to its bindings. usize points to SemanticModelBuilder::bindings vec
    bindings_by_reference_range: FxHashMap<TextSize, BindingIndex>,
    exported: FxHashSet<BindingIndex>,
...
impl SemanticModelBuilder {
    pub fn new(root: AnyJsRoot) -> Self {
        Self {
            ...
            bindings: vec![],
            bindings_by_range: vec![],
            bindings_by_reference_range: FxHashMap::default(),
    ...
    pub fn push_event(&mut self, e: SemanticEvent) {
        ...
        match e {
            ...
            DeclarationFound {
                ...
                // Declarations will be raised in order
                debug_assert!(binding_index.0 as usize == self.bindings.len());

                self.bindings.push(SemanticModelBindingData {
                    kind,
                    index: binding_index.into(),
                    range,
                    references: vec![],
                });
                let binding_id = if kind == BindingKind::Value {
                    BindingIdentifier::value_identifier_from(&range)
                } else {
                    BindingIdentifier::type_identifier_from(&range)
                };
                debug_assert!(self.bindings_by_range.last().and_then(|last| Some(last < &binding_id)).unwrap_or(true));
                self.bindings_by_range.push(binding_id);

                let scope = self.scopes.get_mut(binding_scope_id).unwrap();

                scope.bindings.push(binding_index);
                scope.bindings_by_name.insert(name, binding_index);
            ...
            Read {
                ...
                // SAFETY: this binding id is guaranteed to exist because they were generated by the
                // event extractor
                debug_assert!(binding_index.0 < self.bindings.len());

                let binding = &mut self.bindings[binding_index.0];
                let reference_index = binding.references.len();

                binding.references.push(SemanticModelReference {
                    index: (binding.index, reference_index).into(),
                    range,
                ...
                self.bindings_by_reference_range.insert(range.start(), binding_index);
            }
            HoistedRead {
                range,
                binding_index,
            ...
            Exported { binding_index, .. } => {
                self.exported.insert(binding_index);
            }
        }
    }
    ...
    pub fn build(self) -> SemanticModel {
        let data = SemanticModelData {
            ...
            bindings_by_reference_range: self.bindings_by_reference_range,
