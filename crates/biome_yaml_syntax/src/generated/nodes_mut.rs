...
impl YamlBlockMapping {
    pub fn with_yaml_indent(self, element: YamlIndent) -> Self {
        Self::unwrap_cast(
            self.syntax
                .splice_slots(0usize..=0usize, once(Some(element.into_syntax().into()))),
        )
    ...
    pub fn with_yaml_dedent(self, element: Option<YamlDedent>) -> Self {
        Self::unwrap_cast(self.syntax.splice_slots(
            2usize..=2usize,
            once(element.map(|element| element.into_syntax().into())),
        ))
...
impl YamlBlockSequence {
    pub fn with_yaml_indent(self, element: Option<YamlIndent>) -> Self {
        Self::unwrap_cast(self.syntax.splice_slots(
            0usize..=0usize,
            once(element.map(|element| element.into_syntax().into())),
        ))
