...
impl YamlCompactMapping {
    pub fn with_inline_entry(self, element: AnyYamlBlockMapEntry) -> Self {
        Self::unwrap_cast(
            self.syntax
                .splice_slots(0usize..=0usize, once(Some(element.into_syntax().into()))),
        )
    }
    pub fn with_indented_entries(self, element: Option<YamlCompactMappingIndented>) -> Self {
        Self::unwrap_cast(self.syntax.splice_slots(
            1usize..=1usize,
            once(element.map(|element| element.into_syntax().into())),
        ))
...
impl YamlCompactMappingIndented {
    ...
    pub fn with_dedent_token(self, element: SyntaxToken) -> Self {
        Self::unwrap_cast(
            self.syntax
...
impl YamlCompactSequence {
    pub fn with_inline_entry(self, element: YamlBlockSequenceEntry) -> Self {
        Self::unwrap_cast(
            self.syntax
                .splice_slots(0usize..=0usize, once(Some(element.into_syntax().into()))),
        )
    }
    pub fn with_indented_entries(self, element: Option<YamlCompactSequenceIndented>) -> Self {
        Self::unwrap_cast(self.syntax.splice_slots(
            1usize..=1usize,
            once(element.map(|element| element.into_syntax().into())),
        ))
