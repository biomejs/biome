    ...
    ($ node : expr , $ pattern : pat => $ body : expr) => {
        match $node {
            node => match $crate::YamlSyntaxNode::kind(&node) {
                ...
                $crate::YamlSyntaxKind::YAML_DEDENT => {
                    let $pattern = unsafe { $crate::YamlDedent::new_unchecked(node) };
                    $body
                }
                ...
                $crate::YamlSyntaxKind::YAML_INDENT => {
                    let $pattern = unsafe { $crate::YamlIndent::new_unchecked(node) };
                    $body
                }
