...
use biome_js_syntax::{
    ...
    JsCallArgumentList, JsCallArguments, JsCallExpression, JsCaseClause, JsClassDeclaration,
    ...
    JsIdentifierBinding, JsIfStatement, JsInitializerClause, JsInstanceofExpression,
    JsLogicalExpression, JsLogicalOperator, JsMethodObjectMember, JsNewExpression,
    JsObjectBindingPattern, JsObjectExpression, JsParameterList, JsParameters,
    JsParenthesizedExpression, JsPropertyClassMember, JsPropertyObjectMember,
    JsReferenceIdentifier, JsRestParameter, JsReturnStatement, JsSetterObjectMember,
    JsStaticMemberAssignment, JsSwitchStatement, JsSyntaxKind, JsSyntaxNode, JsSyntaxToken,
...
use crate::{
    ...
    IndexedAccessType, Interface, Intersection, Literal, MemberEqualsPredicate, Module,
    NamedFunctionParameter, Namespace, NarrowingInvalidationKind, NarrowingPredicate, Object, Path,
    ...
    TypeofLogicalOrExpression, TypeofNarrowedExpression, TypeofNewExpression,
    ...
    pub fn from_js_reference_identifier(
        ...
        match name.text() {
            ...
            _ => {
                ...
                // The outermost guard applies first, so it ends up deepest in
                // the nesting.
                let mut narrowed = Self::reference(TypeReference::from_name(scope_id, name));
                for predicate in predicates.into_iter().rev() {
                    let ty = resolver.reference_to_owned_data(narrowed);
                    narrowed = Self::from(TypeofExpression::Narrowed(TypeofNarrowedExpression {
                        ty,
                        predicate,
                    }));
                }
    ...
    fn member_write_invalidated_within(&mut self, node: &JsSyntaxNode) -> bool {
        ...
        let invalidated = node.descendants().any(|descendant| {
            let object = match descendant.kind() {
                ...
                JsSyntaxKind::JS_UNARY_EXPRESSION => JsUnaryExpression::cast(descendant)
                    .filter(|unary| unary.operator() == Ok(JsUnaryOperator::Delete))
                    .and_then(|unary| unary.argument().ok())
                    .and_then(|argument| match argument.omit_parentheses() {
