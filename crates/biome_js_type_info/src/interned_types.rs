...
impl<'db> TypeData<'db> {
    ...
    pub fn is_promise_instance(self, db: &'db dyn TypeDb) -> Option<bool> {
        ...
        Some(instance.ty(db).is_promise_class(db))
    }

    pub fn is_array_class(self, db: &'db dyn TypeDb) -> bool {
        self == Self::GlobalType(ARRAY_ID_GLOBAL_TYPE_ID)
            || matches!(self, Self::Class(class) if class.is_builtin(db) && class.name(db).as_ref().is_some_and(|name| name.text() == "Array"))
    }

    pub fn is_promise_class(self, db: &'db dyn TypeDb) -> bool {
        self == Self::GlobalType(PROMISE_ID_GLOBAL_TYPE_ID)
            || matches!(self, Self::Class(class) if class.is_builtin(db) && class.name(db).as_ref().is_some_and(|name| name.text() == "Promise"))
    }
