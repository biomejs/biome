...
pub fn is_function_boundary(kind: JsSyntaxKind) -> bool {
    AnyFunctionLike::can_cast(kind) || is_non_async_function_boundary(kind)
}

/// Returns `true` if the node kind is a function-like scope boundary that can
/// never be `async`: a getter, a setter, or a static initialization block.
pub fn is_non_async_function_boundary(kind: JsSyntaxKind) -> bool {
    matches!(
        kind,
