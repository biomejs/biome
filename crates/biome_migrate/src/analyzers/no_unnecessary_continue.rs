...
declare_migration! {
    pub(crate) NoUnncesseraryContinue {
        version: "2.0.0",
        name: "noUnnecessaryContinue",
    }
}

impl Rule for NoUnncesseraryContinue {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        if text.text() == "noUnnecessaryContinue" {
            return Some(());
        }
    ...
    fn diagnostic(ctx: &RuleContext<Self>, _: &Self::State) -> Option<RuleDiagnostic> {
        Some(RuleDiagnostic::new(
            ...
            markup! {
                "The rule "<Emphasis>"noUnnecessaryContinue"</Emphasis>" has been renamed "<Emphasis>"noUselessContinue"</Emphasis>"."
            }
                .to_owned(),
        ))
    }
    ...
    fn action(ctx: &RuleContext<Self>, _state: &Self::State) -> Option<MigrationAction> {
        ...
        let new_member_name = json_member_name(json_string_literal("noUselessContinue"));
