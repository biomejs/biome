...
use biome_astro_syntax::{AstroLanguage, AstroSyntaxNode, AstroSyntaxToken};
use biome_rowan::AstNode;
use comments::AstroCommentStyle;
use context::AstroFormatContext;
pub use context::AstroFormatOptions;
use cst::FormatAstroSyntaxNode;
...
/// Formats an Astro file based on its features.
///
/// It returns a [Formatted] result, which the user can use to override a file.
pub fn format_node(
    options: AstroFormatOptions,
    root: &AstroSyntaxNode,
) -> FormatResult<Formatted<AstroFormatContext>> {
    biome_formatter::format_node(root, AstroFormatLanguage::new(options))
}
...
#[derive(Debug, Clone)]
pub struct AstroFormatLanguage {
    options: AstroFormatOptions,
}

impl AstroFormatLanguage {
    pub fn new(options: AstroFormatOptions) -> Self {
        Self { options }
    }
}

impl FormatLanguage for AstroFormatLanguage {
    type SyntaxLanguage = AstroLanguage;
    type Context = AstroFormatContext;
    type FormatRule = FormatAstroSyntaxNode;
    ...
    fn create_context(
        ...
        let comments = Comments::from_node(root, &AstroCommentStyle, source_map.as_ref());
        AstroFormatContext::new(self.options, comments).with_source_map(source_map)
    }
}

pub(crate) type AstroFormatter<'buf> = Formatter<'buf, AstroFormatContext>;
pub(crate) type FormatAstroSyntaxToken = FormatToken<AstroFormatContext>;

// Rule for formatting an Astro [AstNode].
pub(crate) trait FormatNodeRule<N>
where
    N: AstNode<Language = AstroLanguage>,
{
    fn fmt(&self, node: &N, f: &mut AstroFormatter) -> FormatResult<()> {
    ...
    fn fmt_node(&self, node: &N, f: &mut AstroFormatter) -> FormatResult<()> {
        self.fmt_fields(node, f)?;
        Ok(())
    }

    /// Formats the node's fields.
    fn fmt_fields(&self, item: &N, f: &mut AstroFormatter) -> FormatResult<()>;

    /// Returns `true` if the node has a suppression comment and should use the same formatting as in the source document.
    fn is_suppressed(&self, node: &N, f: &AstroFormatter) -> bool {
    ...
    fn fmt_leading_comments(&self, node: &N, f: &mut AstroFormatter) -> FormatResult<()> {
    ...
    fn fmt_dangling_comments(&self, node: &N, f: &mut AstroFormatter) -> FormatResult<()> {
    ...
    fn fmt_trailing_comments(&self, node: &N, f: &mut AstroFormatter) -> FormatResult<()> {
...
impl AsFormat<AstroFormatContext> for AstroSyntaxToken {
    type Format<'a> = FormatRefWithRule<'a, Self, FormatAstroSyntaxToken>;

    fn format(&self) -> Self::Format<'_> {
        FormatRefWithRule::new(self, FormatAstroSyntaxToken::default())
    }
}

impl IntoFormat<AstroFormatContext> for AstroSyntaxToken {
    type Format = FormatOwnedWithRule<Self, FormatAstroSyntaxToken>;

    fn into_format(self) -> Self::Format {
        FormatOwnedWithRule::new(self, FormatAstroSyntaxToken::default())
    }
}
