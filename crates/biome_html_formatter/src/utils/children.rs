...
use biome_rowan::{SyntaxNodeCast, SyntaxResult, TextLen, TextRange, TextSize, TokenText};
    ...
    for child in children {
        match child {
            AnyHtmlElement::HtmlContent(text) => {
                ...
                let is_suppressed = f.comments().is_suppressed(&node);
                if is_suppressed {
                    // SAFETY: we already know that the node is a HtmlContent node, and therefore can be cast to AnyHtmlElement
                    builder.entry(HtmlChild::Verbatim(node.cast().unwrap()));
                    continue;
                }
                ...
                for comment in f.comments().leading_dangling_trailing_comments(&node) {
            ...
            Some(last @ (HtmlChild::EmptyLine | HtmlChild::Newline | HtmlChild::Whitespace)) => {
                if matches!(child, HtmlChild::Whitespace) {
                    *last = child;
                    ...
                    child,
                    HtmlChild::NonText(_) | HtmlChild::Word(_) | HtmlChild::Comment(_)
                ) {
