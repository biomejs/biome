...
Please note that this derive macro requires that the type of each field also implements
`Deserializable`.

The current implementation of the macro only supports enums where none of variants have custom
fields, and structs with named fields. Other complex types will require a manual implementation of
the `Deserializable` trait. See below for some more examples.
