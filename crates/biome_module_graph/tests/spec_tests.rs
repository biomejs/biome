...
use biome_module_graph::js_module_info::MAX_ALIAS_CHAIN_DEPTH;
...
fn test_resolve_exports() {
    ...
    let data = db
        .js_module_info_for_path(Utf8Path::new("/src/reexports.ts"))
        .unwrap();
...
fn test_resolve_self_referential_alias_does_not_loop() {
    // A self-referential alias (`type Loop = Id<Loop>`) is malformed TypeScript, but the resolver
    // must still terminate: the cycle guard leaves it opaque instead of looping on it.
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        "type Id<T> = T;\ntype Loop = Id<Loop>;\nexport function self(): Id<Loop> { return null as never; }\n",
    );
    ...
    let id = resolver
        .resolve_type_of(&Text::new_static("self"), ScopeId::GLOBAL)
        .expect("self not found");
    let ty = resolver.resolved_type_for_id(id);
    // Resolving the return annotation must terminate without looping on the cycle.
    let _ = ty
        .as_function()
        .expect("self must be a function")
        .return_type
        .as_type()
        .and_then(|return_ty| ty.resolve(return_ty));
}

#[test]
fn test_resolve_mutual_circular_alias_does_not_loop() {
    // Mutually recursive aliases (`type A = Id<B>; type B = Id<A>`) are malformed TypeScript; the
    // resolver's cycle guard must leave them opaque and terminate instead of looping.
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        "type Id<T> = T;\ntype A = Id<B>;\ntype B = Id<A>;\nexport function mutual(): Id<A> { return null as never; }\n",
    );
    ...
    let id = resolver
        .resolve_type_of(&Text::new_static("mutual"), ScopeId::GLOBAL)
        .expect("mutual not found");
    let ty = resolver.resolved_type_for_id(id);
    let _ = ty
        .as_function()
        .expect("mutual must be a function")
        .return_type
        .as_type()
        .and_then(|return_ty| ty.resolve(return_ty));
}
...
#[test]
fn test_resolve_wrong_arity_generic_alias_stays_opaque() {
    // `TwoParams<string>` supplies only `A`; `B` has no argument and no default, so the alias
    // cannot be instantiated. The instantiation aborts and the alias stays an opaque `InstanceOf`
    // of its body rather than collapsing to a bound union that silently drops `B` (and resolving
    // must not panic).
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        "type TwoParams<A, B> = A | B;\nexport function get(): TwoParams<string> { return \"hello\"; }\n",
    );
    ...
    assert!(
        matches!(
            return_ty
                .resolved_data()
                .map(|data| data.as_raw_data().clone()),
            Some(TypeData::InstanceOf(_) | TypeData::Union(_)),
        ),
        "wrong-arity alias must stay opaque (uninstantiated), got: {return_ty:?}",
    );
}
...
fn test_resolve_nested_generic_alias_does_not_leak() {
    ...
    with_resolved_return_type(fs, &["/src/index.ts"], "getValue", |return_ty| {
        ...
        assert!(
            return_ty.has_variant(|ty| ty.is_undefined()),
            "expected an `undefined` variant from the nested `Inner<T>`, got: {return_ty:?}",
        );
        assert!(
...
"#,
    ...
    assert!(
        css_info.classes.values().any(|t| t.text() == "button"),
        "styles.css must define class 'button'"
    );
    assert!(
        css_info.classes.values().any(|t| t.text() == "header"),
...
/// [`HtmlEmbeddedContent::Css`] ready for [`resolve_html_module`].
...
fn test_html_self_closing_element_class_references_are_collected() {
    ...
    let layout = ProjectLayout::default();
    let db = build_html_db(&fs, &layout, &[(&html_path, html_root, vec![])]);
...
/// Builds a module db by indexing all given files through a real
...
#[test]
fn test_generic_type_alias_any_argument_collapses_nested_union() {
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        r#"
        type Box<T> = { value: T | null };

        export function getValue(): Box<any> {
            return { value: "biome" };
        }
        "#,
    );
    ...
    with_resolved_return_type(fs, &["/src/index.ts"], "getValue", |return_ty| {
        let value_ty = return_ty
            ...
            .expect("expected a `value` member");
        assert!(
            value_ty.is_any_keyword(),
            "expected nested `any | null` to collapse to `any`, got: {value_ty:?}",
        );
    });
}

#[test]
fn test_generic_conditional_alias_with_constant_condition() {
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        r#"
        type ConstCondition<T> = string extends string ? T : null;

        export function getValue(): ConstCondition<string> {
            return "biome";
        }
        "#,
    );
    ...
    with_resolved_return_type(fs, &["/src/index.ts"], "getValue", |return_ty| {
        assert!(
            return_ty.is_string_or_string_literal(),
            "constant conditional aliases must resolve to the selected branch, got: {return_ty:?}",
        );
    });
}

#[test]
fn test_generic_function_conditional_does_not_select_bare_parameter_branch() {
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        r#"
        export function getValue<T>(): T extends T ? T : null {
            throw 0;
        }
        "#,
    );
    ...
    with_resolved_return_type(fs, &["/src/index.ts"], "getValue", |return_ty| {
        assert!(
            return_ty.has_variant(|ty| ty.is_null()),
            "a bare generic parameter conditional must not collapse to the true branch, got: {return_ty:?}",
        );
    });
}

#[test]
fn test_generic_conditional_alias_does_not_select_alias_wrapped_parameter_branch() {
    let fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        r#"
        type Id<T> = T;
        type Alias<T> = Id<T> extends Id<T> ? 1 : 2;

        export function getValue(): Alias<never> {
            throw 0;
        }
        "#,
    );
    ...
    with_resolved_return_type(fs, &["/src/index.ts"], "getValue", |return_ty| {
        assert!(
            ...
            "an alias-wrapped generic parameter conditional must not collapse to the true branch, got: {return_ty:?}",
        );
    });
}
