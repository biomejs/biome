    ...
    quote! {
        impl #generics biome_deserialize::Deserializable for #ident #generics #trait_bounds{
            fn deserialize(
                ...
                let result = match biome_deserialize::Text::deserialize(ctx, value, name)?.text() {
...
fn generate_deserializable_from(
    ...
    let validator = if data.with_validator {
        quote! {
            if !biome_deserialize::DeserializableValidator::validate(&result, ctx, name, range) {
                return None;
            }
        }
    } else {
    ...
    quote! {
        impl #generics biome_deserialize::Deserializable for #ident #generics #trait_bounds {
            fn deserialize(
                ...
                let result: #try_from = biome_deserialize::Deserializable::deserialize(ctx, value, name)?;
