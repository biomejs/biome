...
impl TwCandidate {
    ...
    pub fn as_fields(&self) -> TwCandidateFields {
        TwCandidateFields {
...
impl Serialize for TwCandidate {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
impl AstNode for TwCandidate {
    type Language = Language;
    const KIND_SET: SyntaxKindSet<Language> =
        SyntaxKindSet::from_raw(RawSyntaxKind(TW_CANDIDATE as u16));
    fn can_cast(kind: SyntaxKind) -> bool {
        kind == TW_CANDIDATE
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
...
impl std::fmt::Debug for TwCandidate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        thread_local! { static DEPTH : std :: cell :: Cell < u8 > = const { std :: cell :: Cell :: new (0) } };
        let current_depth = DEPTH.get();
        let result = if current_depth < 16 {
            DEPTH.set(current_depth + 1);
            f.debug_struct("TwCandidate")
            ...
            f.debug_struct("TwCandidate").finish()
        };
        DEPTH.set(current_depth);
        result
    }
}
impl From<TwCandidate> for SyntaxNode {
    fn from(n: TwCandidate) -> Self {
        n.syntax
    }
}
impl From<TwCandidate> for SyntaxElement {
    fn from(n: TwCandidate) -> Self {
        n.syntax.into()
    }
}
...
impl AstNode for AnyTwValue {
    ...
    fn can_cast(kind: SyntaxKind) -> bool {
        matches!(
            kind,
            TW_ARBITRARY_VALUE | TW_CSS_VARIABLE_VALUE | TW_NAMED_VALUE
        )
    }
    fn cast(syntax: SyntaxNode) -> Option<Self> {
        let res = match syntax.kind() {
...
impl std::fmt::Display for TwCandidate {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        std::fmt::Display::fmt(self.syntax(), f)
    }
}
...
biome_rowan::declare_node_union! { pub AnyTwBogusNode = TwBogus | TwBogusCandidate | TwBogusModifier | TwBogusVariant }
