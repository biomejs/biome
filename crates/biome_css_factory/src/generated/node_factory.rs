...
pub fn css_unordered_testing(
    size: CssRegularDimension,
    family: CssColor,
) -> CssUnorderedTestingBuilder {
    CssUnorderedTestingBuilder {
        size,
        family,
        style: None,
...
pub struct CssUnorderedTestingBuilder {
    size: CssRegularDimension,
    family: CssColor,
    style: Option<CssDashedIdentifier>,
    variant: Option<CssCustomIdentifier>,
    weight: Option<CssNumber>,
    stretch: Option<CssString>,
    slash_token: Option<SyntaxToken>,
    line_height: Option<CssBinaryExpression>,
}
impl CssUnorderedTestingBuilder {
    pub fn with_style(mut self, style: CssDashedIdentifier) -> Self {
        self.style = Some(style);
        self
    }
    pub fn with_variant(mut self, variant: CssCustomIdentifier) -> Self {
        self.variant = Some(variant);
        self
    }
    pub fn with_weight(mut self, weight: CssNumber) -> Self {
        self.weight = Some(weight);
        self
    }
    pub fn with_stretch(mut self, stretch: CssString) -> Self {
    ...
    pub fn with_line_height(mut self, line_height: CssBinaryExpression) -> Self {
        self.line_height = Some(line_height);
        self
    }
    pub fn build(self) -> CssUnorderedTesting {
        CssUnorderedTesting::unwrap_cast(SyntaxNode::new_detached(
            CssSyntaxKind::CSS_UNORDERED_TESTING,
            [
                self.style
                    .map(|token| SyntaxElement::Node(token.into_syntax())),
                self.variant
                ...
                self.stretch
                    .map(|token| SyntaxElement::Node(token.into_syntax())),
                Some(SyntaxElement::Node(self.size.into_syntax())),
                self.slash_token.map(|token| SyntaxElement::Token(token)),
                self.line_height
                    .map(|token| SyntaxElement::Node(token.into_syntax())),
                Some(SyntaxElement::Node(self.family.into_syntax())),
            ],
        ))
    }
}
