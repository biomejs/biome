    ...
    fn make_syntax(
        ...
        match kind {
            ...
            CSS_HIGHLIGHT_ELEMENT_NAME => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        CSS_HIGHLIGHT_ELEMENT_NAME.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(CSS_HIGHLIGHT_ELEMENT_NAME, children)
            }
            CSS_ID_SELECTOR => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<2usize> = RawNodeSlots::default();
            ...
            CSS_PSEUDO_ELEMENT_HIGHLIGHT => {
                ...
                let mut current_element = elements.next();
                if let Some(element) = &current_element {
                    if CssHighlightElementName::can_cast(element.kind()) {
                        slots.mark_present();
                        current_element = elements.next();
                    }
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        CSS_PSEUDO_ELEMENT_HIGHLIGHT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(CSS_PSEUDO_ELEMENT_HIGHLIGHT, children)
            }
