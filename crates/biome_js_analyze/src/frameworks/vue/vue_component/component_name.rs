...
impl AnyVueComponent {
    ...
    pub fn component_name(&self) -> Option<TokenText> {
        ...
        for member in object_expression.members().into_iter().flatten() {
            if let AnyJsObjectMember::JsPropertyObjectMember(property) = member {
                if property
                    .name()
                    .ok()
                    .and_then(|n| n.name()).is_none_or(|n| n != "name")
                {
                    continue;
                };
                ...
                if let Ok(value_expr) = property.value() {
                    ...
                    if let Some(str_lit) = value_expr
                        ...
                        .and_then(|lit| lit.inner_string_text().ok())
                    {
                        return Some(str_lit);
                    }
                }
...
impl PartialEq<str> for VueComponentName<'_> {
    fn eq(&self, other: &str) -> bool {
        match self {
            VueComponentName::FromComponent(name) => *name == other,
...
impl PartialOrd<str> for VueComponentName<'_> {
    fn partial_cmp(&self, other: &str) -> Option<std::cmp::Ordering> {
        match self {
            VueComponentName::FromComponent(name) => name.text().partial_cmp(other),
...
impl Deref for VueComponentName<'_> {
    ...
    fn deref(&self) -> &Self::Target {
        match self {
            VueComponentName::FromComponent(name) => name.text(),
