...
fn get_rule_options(analyzer_options: &AnalyzerOptions) -> UseSortedClassesOptions {
...
impl Visitor for StringLiteralInAttributeVisitor {
    ...
    fn visit(
        ...
        let options = get_rule_options(ctx.options);
        match event {
...
fn find_class_order_index(class_order: &Vec<String>, class_name: &str) -> Option<usize> {
    ...
    for (i, spec) in class_order.iter().enumerate() {
        match get_class_order_match(spec, class_name) {
...
fn sort_class_name(class_name: &str, class_order: &Vec<String>) -> String {
    let classes = class_name.split_whitespace().collect::<Vec<&str>>();
    let mut unordered_classes: Vec<&str> = Vec::new();
    let mut class_order_map: HashMap<usize, Vec<&str>> = HashMap::new();
    ...
    for i in 0..class_order.len() {
        if let Some(classes) = class_order_map.get(&i) {
...
impl Rule for UseSortedClasses {
    ...
    fn run(ctx: &RuleContext<Self>) -> Option<Self::State> {
        ...
        let class_order = get_class_order(&options.preset, &options.class_order);
        let sorted_value = sort_class_name(value.as_str(), &class_order);
...
impl DeserializationVisitor for UseSortedClassesOptionsVisitor {
    ...
    fn visit_map(
        ...
        for (key, value) in members.flatten() {
            ...
            match key_text.text() {
                "preset" => {
                    // TODO: actually retrieve the value
                    result.preset = ClassOrderPreset::Tailwind;
                }
                "attributes" => {
                ...
                "class_order" => {
                    if let Some(class_order) =
                        Deserializable::deserialize(&value, &key_text, diagnostics)
                    {
                        result.class_order = class_order;
                    }
                }
                unknown_key => {
                    const ALLOWED_KEYS: &[&str] = &["behaviorExceptions"];
...
#[derive(Debug, Default, Clone)]
pub enum ClassOrderPreset {
    NoPreset,
    #[default]
    Tailwind,
}

fn get_class_order(preset: &ClassOrderPreset, class_order: &Vec<String>) -> Vec<String> {
    if !class_order.is_empty() {
        return class_order.clone();
    }
    match preset {
        ClassOrderPreset::NoPreset => {
            panic!("TODO: no preset error message")
        }
        ClassOrderPreset::Tailwind => {
            vec![
