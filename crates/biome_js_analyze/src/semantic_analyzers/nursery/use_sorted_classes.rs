...
fn parse_class(class_name: &str) -> UtilityData {
    // state
    ...
    for (index, c) in class_name.chars().enumerate() {
        ...
        match c {
            '[' => {
                ...
                } else if let None = quoted_arbitrary_block_type {
                    arbitrary_block_depth += 1;
                }
            }
            '\'' | '"' | '`' => {
                if at_arbitrary_block_start {
                    ...
                    quoted_arbitrary_block_type = Some(quote.clone());
