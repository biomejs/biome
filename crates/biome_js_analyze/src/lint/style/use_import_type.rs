...
declare_lint_rule! {
    ...
    /// it can't always detect that an import used as a type is also used as a value in the code generated by a decorator.
    /// This can lead Biome to suggest importing some imports as type, which are actually used as value at runtime.
    ///
    /// If your project relies on metadata emitted for decorated constructor parameters,
    /// enable the `preserveDecoratorMetadata` option to keep those imports as values.
    /// If your decorators use a different pattern that still needs type annotations at runtime,
