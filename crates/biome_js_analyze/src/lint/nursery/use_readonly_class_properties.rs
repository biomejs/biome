...
use biome_js_syntax::{
    ...
    JsSyntaxToken, JsThisExpression, TextRange, TsAccessibilityModifier, TsPropertyParameter,
    TsReadonlyModifier,
};
use biome_rowan::{
...
impl Rule for UseReadonlyClassProperties {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        let mutable_class_property_members = members
            .iter()
            .filter_map(|member| filter_mutable_class_property(&member, private_only));
...
/// Filters mutable class properties (excluding `static` and `accessor`), optionally restricted to `private` ones, from a given class member.
fn filter_mutable_class_property(
    ...
    members
        ...
        .filter_map(|member| match member {
            ...
            AnyJsClassMember::JsConstructorClassMember(_) => {
                // todo!("constructor, deal self executing immediate fn");
                None
            }
            _ => None,
        })
...
fn find_all_assignment_names(node: &SyntaxNode<JsLanguage>) -> impl Iterator<Item = Text> + '_ {
    node.children().flat_map(|child| {
        ...
        {
            std::iter::once(extract_static_member_assignment_name(&assignment))
...
fn extract_range_and_text(property_or_param: PropOrParam) -> Option<TextAndRange> {
    ...
    if let Some(PropOrParam::TsPropertyParameter(parameter)) =
        PropOrParam::cast(property_or_param.clone().into())
    {
        let name = parameter.formal_parameter().ok()?.as_js_formal_parameter()?.binding().ok()?;
