...
declare_lint_rule! {
    ...
    /// ORM instances. The option supports both simple identifiers (e.g., `db`) and member
    /// expressions (e.g., `context.db`).
    ///
    /// Configuring `"db"` will match any expression that ends with `.db`:
    /// - `db.update()` ✓
    /// - `context.db.update()` ✓
    /// - `nested.context.db.update()` ✓
    ///
    /// ```json,options
    /// {
    ///   "options": {
