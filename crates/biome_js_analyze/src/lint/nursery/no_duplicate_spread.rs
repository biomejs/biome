...
use biome_rule_options::no_duplicate_spread::NoDuplicateSpreadOptions;
...
declare_lint_rule! {
    ...
    /// ```
    ///
    pub NoDuplicateSpread {
        version: "next",
        name: "noDuplicateSpread",
        language: "js",
        recommended: false,
...
impl Rule for NoDuplicateSpread {
    type Query = Ast<NoDuplicateSpreadQuery>;
    type State = String;
    type Signals = Option<Self::State>;
    type Options = NoDuplicateSpreadOptions;

    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        let binding = ctx.query();

        match binding {
            NoDuplicateSpreadQuery::JsxOpeningElement(node) => {
                let attributes = node.attributes();
                validate_attributes(&attributes)
            }
            NoDuplicateSpreadQuery::JsxSelfClosingElement(node) => {
                let attributes = node.attributes();
                validate_attributes(&attributes)
            }
        }
