...
impl Rule for NoReactLeakedIntersectionObserver {
    ...
    type State = NoReactLeakedIntersectionObserverState;
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        {
            if !is_new_intersection_observer(&new_expr) {
                continue;
            }
                ...
                .any(|ancestor| JsVariableDeclarator::can_cast(ancestor.kind()))
            {
                return Some(NoReactLeakedIntersectionObserverState {
                    ...
                    kind: IntersectionObserverLeakKind::UnexpectedFloatingInstance,
                });
            }
    ...
    fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
        let message = match state.kind {
            IntersectionObserverLeakKind::ExpectedDisconnectInControlFlow => {
                markup! {
                    "Dynamically observed elements should be cleaned up with "<Emphasis>"disconnect()"</Emphasis>" in the cleanup function."
                }
            }
            IntersectionObserverLeakKind::ExpectedDisconnectOrUnobserveInCleanup => {
                markup! {
                    "An "<Emphasis>"IntersectionObserver"</Emphasis>" created in an effect must be disconnected or fully unobserved in the cleanup function."
                }
            }
            IntersectionObserverLeakKind::UnexpectedFloatingInstance => {
                markup! {
                    "Assign this "<Emphasis>"IntersectionObserver"</Emphasis>" instance to a variable so it can be cleaned up in the effect cleanup function."
                }
            }
        };
...
pub enum IntersectionObserverLeakKind {
...
#[derive(Debug, Clone, Copy)]
pub struct NoReactLeakedIntersectionObserverState {
    range: TextRange,
    kind: IntersectionObserverLeakKind,
}

fn is_new_intersection_observer(new_expr: &JsNewExpression) -> bool {
