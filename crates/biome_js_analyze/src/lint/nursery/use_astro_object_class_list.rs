...
fn is_direct_class_list_value(conditional: &JsConditionalExpression) -> bool {
    let Some(root) = conditional
        .syntax()
        .ancestors()
    ...
    if root_expression.syntax() == conditional.syntax() {
        return true;
    }
    ...
    array.elements().iter().any(|element| {
        matches!(
            ...
            Ok(AnyJsArrayElement::AnyJsExpression(expression))
                if expression.clone().omit_parentheses().syntax() == conditional.syntax()
        )
    })
}
