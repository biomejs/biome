...
impl Rule for NoReactLeakedTimeout {
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        {
            if !is_set_timeout_call(&call) {
                continue;
            }

            // Check if it's a bare setTimeout call (not assigned to a variable)
            if !call
                .syntax()
                .ancestors()
                .any(|ancestor| JsVariableDeclarator::can_cast(ancestor.kind()))
            {
                // Bare setTimeout call without assignment - needs clearTimeout
                if !has_clear_timeout_in_cleanup(callback_node, None) {
                    return Some(call.range());
                }

            // setTimeout assigned to a variable - check if that variable is cleared
            } else if let Some(var_name) = get_assigned_variable_name(&call)
                && !has_clear_timeout_in_cleanup(callback_node, Some(&var_name))
            {
                return Some(call.range());
            }
        }
    ...
    fn diagnostic(_: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
        Some(RuleDiagnostic::new(
            ...
            state,
            markup! {
                "A "<Emphasis>"setTimeout"</Emphasis>" created in an effect must be cleared in the cleanup function."
            },
        )
        .note(markup! {
            "Not clearing the timeout can cause memory leaks and trigger state updates on an unmounted component."
        })
        .note(markup! {
            "Return a cleanup function from the effect that calls "<Emphasis>"clearTimeout"</Emphasis>" with the timeout ID."
        }))
    }
}

fn is_set_timeout_call(call: &JsCallExpression) -> bool {
    let Ok(callee) = call.callee() else {
        return false;
    };

    // Handle normal `setTimeout(...)` and `window.setTimeout(...)` calls
    if let Some(name) = callee.get_callee_member_name() {
        return name.text_trimmed() == "setTimeout";
    }

    // Handle `(setTimeout as any)(...)` — strip parentheses and TS type assertions
