...
#[cfg(test)]
mod tests {
    use super::{could_be_error, is_reject_call};
    ...
    #[test]
    fn long_logical_chains() {
        for operator in ["||", "??"] {
            for (last, expected) in [("0", false), ("error", true)] {
                let source = format!(
                    "Promise.reject({}{last});",
                    format!("0 {operator} ").repeat(3_000)
                );
                let parsed = parse(
                    &source,
                ...
                let argument = call
                    ...
                    .unwrap()
                    .unwrap();
                assert_eq!(is_reject_call(&call, &model), Some(true));
                assert_eq!(
                    could_be_error(argument.as_any_js_expression().unwrap().clone(), &model),
                    Some(expected)
                );
            }
        }
