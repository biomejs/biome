...
/// Return the canonical-cased attribute name when the given `name` matches an entry
/// in `DOM_PROPERTIES_IGNORE_CASE` ignoring ASCII case; otherwise return `name` unchanged.
///
/// This is used to normalize attribute lookups for properties that should be treated
/// case-insensitively while preserving the canonical string from the lookup table.
///
/// # Examples
///
/// ```
/// // Unmatched names are returned unchanged
/// assert_eq!(normalize_attribute_case("unknown-attr"), "unknown-attr");
/// ```
...
/// Determine if a string is a valid HTML `data-*` attribute name.
///
/// The name is considered valid when it:
/// - begins with the `data-` prefix,
/// - is not a `data-xml*` attribute (case-insensitive),
/// - has at least one character after `data-`, and
/// - does not contain a `:` character in the part after `data-`.
///
/// # Returns
///
/// `true` if the name is a valid `data-*` attribute, `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(is_valid_data_attribute("data-user-id"));
/// assert!(is_valid_data_attribute("data-foo"));
/// assert!(!is_valid_data_attribute("data-")); // empty name after prefix
/// assert!(!is_valid_data_attribute("aria-label")); // doesn't start with data-
/// assert!(!is_valid_data_attribute("data-xmlfoo")); // disallowed data-xml*
/// assert!(!is_valid_data_attribute("data:foo")); // contains ':'
/// ```
...
/// Checks whether the provided name is a recognized ARIA attribute.
///
/// Returns `true` if the name is a recognized ARIA attribute, `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(is_valid_aria_attribute("aria-label"));
/// assert!(!is_valid_aria_attribute("not-an-aria-attr"));
/// ```
fn is_valid_aria_attribute(name: &str) -> bool {
    ARIA_PROPERTIES.iter().any(|&element| element == name)
}

/// Determines whether a JSX tag name should be treated as a plain HTML tag.

///

/// This returns `true` when `tag_name` follows the HTML tag naming convention

/// used in JSX (starts with a lowercase ASCII letter and does not contain

/// hyphens after the first character) and the element does not provide an

/// `is` attribute (which would indicate a customized built-in element).
...
/// - `node`: the JSX element node to inspect for an `is` attribute.

/// - `tag_name`: the tag name text to validate.
...
/// `true` if `tag_name` is a plain HTML tag name in JSX and `node` has no `is` attribute, `false` otherwise.
...
/// // Pseudocode demonstrating intended usage:

/// // let node: AnyJsxElement = ...;

/// // assert!(is_valid_html_tag_in_jsx(&node, "div"));

/// // assert!(!is_valid_html_tag_in_jsx(&node, "MyComponent"));

/// // assert!(!is_valid_html_tag_in_jsx(&node, "custom-element")); // contains hyphen after first char
...
/// Determine whether a JSX element's tag name is a member name (contains a dot).

///

/// Returns `Some(true)` when the element's name is a member expression (e.g., `Foo.bar`),

/// `Some(false)` when the element has a name that is not a member expression, and

/// `None` if the element's name could not be retrieved.
...
/// // Given a `node: AnyJsxElement`, use `tag_name_has_dot` to check for member names:

/// // let result = tag_name_has_dot(&node);

/// // match result {

/// //     Some(true) => println!("Tag is a member name (contains a dot)"),

/// //     Some(false) => println!("Tag is a plain name"),

/// //     None => println!("Could not determine tag name"),
...
/// # Returns
///
/// `true` if the string contains at least one uppercase character, `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(has_uppercase("Data-Attr"));
/// assert!(!has_uppercase("data-attr"));
/// ```
...
/// Resolve the canonical DOM/React property name for a given attribute string.
///
/// This returns the standard name used in the DOM/React mapping when an attribute
/// is recognized either via the HTML-to-React attribute lookup or by a
/// case-insensitive match against known DOM property names.
///
/// # Examples
///
/// ```
/// // Exact HTML attribute mapped to React prop
/// assert_eq!(get_standard_name("class"), Some("className"));
///
/// // Case-insensitive match against known DOM property names
/// assert_eq!(get_standard_name("VIEWBOX").map(|s| s.to_string()), Some("viewBox".to_string()));
///
/// // Unknown attribute yields None
/// assert_eq!(get_standard_name("some-unknown-attr"), None);
/// ```
...
impl Rule for NoUnknownAttribute {
    ...
    /// Determines whether a JSX attribute is a known/allowed DOM property and returns a diagnostic state when it is not.
    ///
    /// The function examines the JSX attribute from the provided rule context and applies rule options and HTML/ARIA/data-* rules to decide validity:
    /// - Returns `Some(State)` when the attribute should produce a diagnostic:
    ///   - `UnknownProp` when the attribute is not recognized for the element,
    ///   - `UnknownPropWithStandardName` when a canonical React/DOM name exists,
    ///   - `InvalidPropOnTag` when the attribute is valid but not for the current tag,
    ///   - `DataLowercaseRequired` when a `data-*` attribute contains uppercase characters while lowercase is required.
    /// - Returns `None` when the attribute is allowed or explicitly ignored (examples: recognized ARIA attributes, valid `data-*` attributes, attributes listed in options.ignore, non-HTML JSX components, tags with dotted names like `Foo.bar`, and special-cased `fbt`/`fbs` tags).
    ///
    /// # Examples
    ///
    /// ```no_run
    /// // Given a rule context `ctx` for a JSX attribute, run returns an optional diagnostic state:
    /// let signal = NoUnknownAttribute.run(&ctx);
    /// match signal {
    ///     Some(state) => eprintln!("diagnostic: {:?}", state),
    ///     None => println!("attribute is valid or ignored"),
    /// }
    /// ```
    ...
    /// Create a lint diagnostic for a `NoUnknownAttribute` rule state.
    ///
    /// The returned diagnostic describes why a JSX/DOM attribute is invalid or
    /// suggests the correct form when applicable. Each variant of
    /// `NoUnknownAttributeState` maps to a user-facing message and one or more
    /// explanatory notes.
    ///
    /// The function returns `Some(RuleDiagnostic)` for every recognized state.
