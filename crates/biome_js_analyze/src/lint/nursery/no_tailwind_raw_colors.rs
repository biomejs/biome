...
declare_lint_rule! {
    ...
    /// colors instead of their purpose. Design system color names such as `primary`
    ...
    /// are also checked. Untagged templates in helper calls outside JSX class
    /// attributes are not checked. Dynamically constructed class names are not resolved.
...
impl Rule for NoTailwindRawColors {
    ...
    type Signals = SmallVec<[Self::State; 4]>;
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        raw_color_ranges(&root.candidates(), ctx.options())
    }
