    ...
    returns.iter().all(|inferred| {
        ...
        while let Some((annotated, inferred)) = stack.pop() {
            ...
            for inferred_member in inferred_members.iter() {
                ...
                // An `unknown` annotated type accepts any value, so it is never a misleading widening.
                // Checked before the const bail so a const-asserted inferred member is still skipped.
                if matches!(&*annotated_type, TypeData::UnknownKeyword) {
                    continue;
                }
...
/// `true` only on an exact match or a decomposed `Matches`; a merely wider variant does not count.
    ...
    while let Some((ann, inf)) = stack.pop() {
        ...
        match (&*ann, &*inf) {
            ...
            (TypeData::Object(ann_obj), TypeData::Object(inf_obj)) => {
                if !push_object_pairs(&ann, ann_obj, &inf, &inf_obj.members, &mut stack) {
                    return NonUnionCoverage::Mismatch;
                }
            }
            ...
            (TypeData::Object(ann_obj), TypeData::Literal(lit)) => match lit.as_ref() {
                Literal::Object(inf_lit) => {
                    if !push_object_pairs(&ann, ann_obj, &inf, inf_lit.members(), &mut stack) {
                        return NonUnionCoverage::Mismatch;
                    }
                }
                _ => return NonUnionCoverage::Mismatch,
...
fn object_is_vacuously_satisfiable(object: &biome_js_type_info::Object) -> bool {
    object
        .members
        .iter()
        .all(|member| member.is_optional() || member.is_index_signature_with_ty(|_| true))
}

/// Pairs each inferred member with the annotation member of the same name (or the index
/// signature value) and pushes the pair onto `stack`. `inf_members` come from either an object
/// type or an object literal. `false` when a required annotation member is absent or an inferred
/// member does not resolve.
fn push_object_pairs(
    annotated: &Type,
    ann_obj: &biome_js_type_info::Object,
    inferred: &Type,
    inf_members: &[biome_js_type_info::TypeMember],
    ...
    for inferred_member in inf_members {
        let ann_ty = match inferred_member.kind.name() {
            ...
            None if inferred_member.is_index_signature_with_ty(|_| true) => {
    ...
    // A required annotation member missing from the inferred members is a shape mismatch.
...
fn is_union_wider_than_returns(annotated: &Type, returns: &[Type]) -> bool {
    ...
    let has_extra = variants
        .iter()
        .any(|ann_v| returns.iter().all(|ret| variant_definitely_uncovered(ann_v, ret)));
