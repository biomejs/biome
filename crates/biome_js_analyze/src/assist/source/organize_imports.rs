...
declare_source_rule! {
    ...
    /// ```
    ///
    /// Chunks also ends as soon as a statement or a **side-effect import** (also called _bare import_) is encountered.
    /// Each side-effect import consists in a chunk:
    ...
    /// `import C from "c"` forms the second chunk.
    /// The other blank lines are ignored: the two first `import` forms the first chunk and the three last `export` form the third chunk.
    ///
    /// ```js,ignore
    /// // Attached comment 1
    /// import A from "a";
    ...
    /// - `:BUN:`: sources starting by the protocol `bun:` or that corresponds to a builtin Bun module such as `bun`.
    /// - `:NODE:`: sources starting by the protocol `node:` or that corresponds to a builtin Node.js module such as `path`.
    ...
    ///
    /// The sorter also sort named imports, named exports, as well as attributes.
