...
// The trivially-true condition (identical sides) resolves to its true branch, so
// `NonGenericConditional` is `"a" | "b"`, and returning only "a" is misleading.
...
// The assertion pins the return to `JustAsserted<string>` = `string`, so the annotation's
// `null` variant is never produced.
type MaybeNarrowedByAssertion<T> = T | null;
type JustAsserted<T> = T;
function assertedGenericAliasNarrower(): MaybeNarrowedByAssertion<string> {
    return "hello" as JustAsserted<string>;
}
