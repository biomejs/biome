//! This is a generated file. Don't modify it by hand! Run 'cargo codegen formatter' to re-generate the file.

pub(crate) mod any_function;
pub(crate) mod attribute_matcher;
pub(crate) mod attribute_matcher_value;
pub(crate) mod attribute_name;
pub(crate) mod block;
pub(crate) mod container_and_query;
pub(crate) mod container_not_query;
pub(crate) mod container_or_query;
pub(crate) mod container_query_in_parens;
pub(crate) mod container_size_feature_in_parens;
pub(crate) mod container_style_and_query;
pub(crate) mod container_style_in_parens;
pub(crate) mod container_style_not_query;
pub(crate) mod container_style_or_query;
pub(crate) mod container_style_query_in_parens;
pub(crate) mod custom_property;
pub(crate) mod declaration;
pub(crate) mod declaration_important;
pub(crate) mod identifier;
pub(crate) mod keyframes_block;
pub(crate) mod keyframes_body;
pub(crate) mod media_query;
pub(crate) mod media_query_consequent;
pub(crate) mod media_query_feature;
pub(crate) mod media_query_feature_boolean;
pub(crate) mod media_query_feature_compare;
pub(crate) mod media_query_feature_plain;
pub(crate) mod media_query_feature_range;
pub(crate) mod media_query_range;
pub(crate) mod named_namespace_prefix;
pub(crate) mod namespace;
pub(crate) mod nth_offset;
pub(crate) mod parameter;
pub(crate) mod root;
pub(crate) mod rule;
pub(crate) mod simple_function;
pub(crate) mod size_feature_boolean;
pub(crate) mod size_feature_plain;
pub(crate) mod size_feature_range;
pub(crate) mod size_feature_range_comparison;
pub(crate) mod size_feature_range_interval;
pub(crate) mod universal_namespace_prefix;
pub(crate) mod var_function;
pub(crate) mod var_function_value;
