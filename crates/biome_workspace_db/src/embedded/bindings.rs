...
mod tests {
    ...
    impl TestDb {
        ...
        fn insert_bindings(&mut self, bindings: Vec<Vec<EmbeddedBinding>>) {
    ...
    fn get_binding_by_name_finds_matching_binding() {
        ...
        db.insert_bindings(vec![vec![
            binding(&db, "Local", range(0, 5), None),
            binding(&db, "Component", range(10, 19), Some("./Component.vue")),
        ]]);

        let found = get_binding_by_name(&db, InternedBinding::new(&db, token_text("Local")))
    ...
    fn get_binding_with_source_ignores_local_bindings() {
        ...
        assert!(
            get_binding_with_source(&db, InternedBinding::new(&db, token_text("Local"))).is_none()
        );

        let found =
            get_binding_with_source(&db, InternedBinding::new(&db, token_text("Component")))
    ...
    fn get_binding_by_name_is_memoized() {
        let mut db = TestDb::new();
        db.insert_bindings(vec![vec![binding(&db, "Local", range(0, 5), None)]]);
        let name = InternedBinding::new(&db, token_text("Local"));
